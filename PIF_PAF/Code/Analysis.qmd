...
riskfactors <- data_rf %>% 
   mutate(
    ...
    # exposure = ifelse(level == "Meets Recommendations", "Meets_PArecommendations", exposure), 
    # exposure = ifelse(level == "Below Recommendations", "Below_PArecommendations", exposure), 
    ...
    variable = if_else(variable == "physical_activity", "Physical Activity", variable),
  ...
  filter(
    ...
    exposure %in% c("Light_alcohol", "Medium_alcohol","Heavy_alcohol","Non-Drinker","BMI_obese_men","BMI_obese_women","BMI_overweight_men","BMI_overweight_women","Healthy Weight","Current_Smoking_men","Current_Smoking_women","Former_Smoking_men","Former_Smoking_women","Never", "Processed_Meat","Red_Meat", "Fibre", "Meets Recommendations", "Below Recommendations")
  )
...
#PHYSICIAL ACTIVITY
PAtrends <- ggplot(rf_trend_plot %>% filter(variable == "Physical Activity"), aes(x = year, y = value, color = exposure)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
 geom_point(data = rf_trend_plot_invisible_points, alpha = 0) +
  ...
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
  geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "Physical Activity"),
    aes(label = APC_clean, group = exposure),
    nudge_x = 3,
    fill = "white", 
    size = 3,
  ...
  scale_color_manual( #fixing the colors 
    values = c(
      "Below Recommendations" = "#2b9eb3",
      "Meets Recommendations" = "#f8333c"
...
data_rf_imd_clean <- data_rf_imd %>% 
  ...
  mutate(
    ...
    variable = ifelse(variable == "physical_activity","Physical_Activity", variable ),
    level = sub(" .*", "", level)
  )
