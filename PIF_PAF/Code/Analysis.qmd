...
RRconstantlow <- 1.25 # Example RR constant
RRconstant4 <- 1.5
RRconstantmed <- 2
RRconstanthigh <- 5
...
changingpaf_data <- rbind(data.frame(prev = prevalencerange, PAF = PAFlow, RR = "1.25"), data.frame(prev = prevalencerange, PAF = PAF4, RR = "1.5")) |>
  rbind(data.frame(prev = prevalencerange, PAF = PAFmed, RR = "2.0")) |>
  rbind(data.frame(prev = prevalencerange, PAF = PAFhigh, RR = "5.0"))

pafplotall<- ggplot(changingpaf_data, aes(x = prev, y = PAF, colour = RR)) +
  geom_line(size = 1) +
  labs(title = "Change in PAF by RR and Prevalence",
       x = "RF Prevalence",
       y = "PAF") +
  ylim(0, 1) +
  theme_minimal() +
...
pif1_data <- calculate_pif_data(exposurerange, 0.10, ".10")
pif2_data <- calculate_pif_data(exposurerange, 0.20, ".20")
pif3_data <- calculate_pif_data(exposurerange, 0.40, ".40")
pif4_data <- calculate_pif_data(exposurerange, 0.60, ".60")
...
pif_plot <- ggplot(combined_pif_data, aes(x = exposure_diff, y = PIF, color = RR)) +
  ...
  labs(
    title = "Theoretical PIFs with different RR and Starting Exposures",
    x = "Relative Exposure Change (%)",
    y = "PIF",
    color = "Relative Risk"
  ) +
  theme_minimal() +
