...
2.  Montse has suggested industry data for dietary factors, purely for graphic purposes. These should be found where possible. 
...
#Reading in PAF all ages combined data 
data_PAFallages <- read.csv("../Data/PAF_allages.csv")

#Reading in the PIF all ages data 
data_PIFallages <- read.csv("../Data/PIF_allages.csv")
...
ggplot(data_cancer_plot, aes(x = year, y = Rate, fill = sex, colour = sex)) +
  ...
  scale_x_continuous(
    breaks = seq(2009, 2019, by = 1),  # Define the x-axis breaks
    limits = c(2009, 2021)
    )+
...
## Figure 2 - Trends in risk factors table
...
## Figure pre 3 - Current PAFs (2019) : all ages

```{r}
#formatting the PAF all ages to help with join 
data_PAFallages_long <- data_PAFallages |>
  pivot_longer(cols = colnames(data_PAFallages)[!colnames(data_PAFallages) %in% c("sex", "year", "age_group", "variable")], names_to = "cancer_site", values_to = "PAF") |>
...
PAF_allages <- data_PAFallages_long %>%
...
PAFallages_bar_MEN <- ggplot(PAF_allages %>% filter(sex == "Men"), aes(x = cancer_site)) +
  ...
  geom_bar(
    aes(y = attributable_cases/60, fill = "Attributable Cases"),
    stat = "identity", 
  ...
  scale_y_continuous(
    ...
    limits = c(0, 100),
    sec.axis = sec_axis(
      ~ . * 60,  # Adjust the transformation factor as needed
      name = "Case Count"
    ) 
  ) +
  scale_fill_manual(
...
## Figure 3 - Current PAFs (2019) - Comparative Bar Chart
...
## Figure 4 - Current PAFs (2019) - Bar chart by Risk Factor
...
data_attr_cases_bar <- data_paf_long_individual %>% 
  ...
  filter(
    year == 2019 | (year == 2018 & all(PAF[year == 2019] == 0)) #this includes 2019 PAF or, if there is none due to lack of data, then it pulls the 2018 PAF
  ) %>%
  select(
    -count, - Rate
  )
...
```{r}
#creating the table 
data_pifallages_past <- data_PIFallages |>
  mutate(
...
#### Obesity and Colorectal Cancer alternate RR scenario
...
youngmen_obesescenario <- pif_scenarioBMI(riskfactors %>% filter(sex == "Men", age_group == "20-49"))
oldmen_obesescenario <- pif_scenarioBMI(riskfactors %>% filter(sex == "Men", age_group == "50+"))
youngwomen_obesescenario <- pif_scenarioBMI(riskfactors %>% filter(sex == "Women", age_group == "20-49"))
oldwomen_obesescenario <- pif_scenarioBMI(riskfactors %>% filter(sex == "Women", age_group == "50+"))
...
#### RR Scenario Example-will not bes used

This example is for Air Pollution only.
