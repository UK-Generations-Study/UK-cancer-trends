...
source("Functions/rolling_average.R")

# Load in function for rotating axis ticks
source("Functions/axis_ticks_rotation.R")

# Load in function for gt tables from joinpoint output
source("Functions/full_joinpoint_table.R")

# Load in function to plot joinpoint output for RF
source("Functions/plot_joinpoint.R")
...
ggsave("../Output/Plots/TheoreticalPAF.png", plot = pafplotall, bg = "white", height = 6, width = 10)
```
...
pif1_data <- calculate_pif_data(exposurerange, 0.10, ".1")
pif2_data <- calculate_pif_data(exposurerange, 0.20, ".2")
pif3_data <- calculate_pif_data(exposurerange, 0.40, ".4")
pif4_data <- calculate_pif_data(exposurerange, 0.60, ".6")
...
ggsave("../Output/Plots/TheoreticalPIF.png", plot = pif_plot, bg = "white", height = 6, width = 10)
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  plot <- data_joinpoint_AAPC |>
    ...
    fp_set_style(box = c("#38b000", "#ff9500") |> lapply(function(x) gpar(fill = x, col = "#555555")),
...
data_joinpoint_age_group_last <- data_joinpoint_APC_age_group |>
  filter(Model == Segment)

data_cancer_plot_age_group <- merge(data_ALLcancer_age_group, data_joinpoint_age_group_last, by = c("sex", "age_group", "cancer_site"), all.x = T) |>
  mutate(
...
# Need to get rows at join and duplicate with alternate last_segment
data_cancer_plot_age_group_new_rows <- data_cancer_plot_age_group |>
  group_by(sex, age_group, cancer_site) |>
  filter(last_segment) |>
  filter(year == min(year)) |>
  mutate(last_segment = F) |>
...
# Now add back in
data_cancer_plot_age_group <- rbind(data_cancer_plot_age_group, data_cancer_plot_age_group_new_rows)

# Now make FALSE for NA values - JP did not run when 0's were included
data_cancer_plot_age_group <- data_cancer_plot_age_group |>
  mutate(last_segment = if_else(is.na(last_segment), F, last_segment))

# Now create labels
data_cancer_plot_age_group_labels <- data_cancer_plot_age_group |>
  group_by(age_group, cancer_site) |>
  mutate(
    Rate_range = max(Rate) - min(Rate)
  ) |>
...
# Create invisible points to expand y limits
data_cancer_age_group_invisible_points <- data_cancer_plot_age_group |>
  group_by(sex, age_group, cancer_site) |>
  summarise(
...
# Loop through each age group and geneRate a plot
for (site in unique(data_cancer_plot_age_group$cancer_site)) {
  # Filter data for the current age group
  data_subset <- data_cancer_plot_age_group %>% filter(cancer_site == site)
  data_invisible <- data_cancer_age_group_invisible_points %>% filter(cancer_site == site)
  data_label <- data_cancer_plot_age_group_labels %>% filter(cancer_site == site)
  ...
  plot <- ggplot(data_subset, aes(x = year, y = Rate, fill = sex, colour = sex)) +
    ...
    geom_label(data = data_label,
               aes(label = APC_clean), fill = "white", size = 3, show.legend = F) +
    theme_minimal() +
    geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf,
              colour = "black", fill = NA, inherit.aes = FALSE) +
    theme(strip.text = element_text(face = "bold"), 
    ...
    ylab("Age Standardized Rates") +
    xlab("Year") +
    labs(colour = "Sex", fill = "Sex", linetype = "Last Join") +
...
# Display each plot sepaRately
for (s in unique(data_cancer_plot_age_group$cancer_site)) {
...
data_cancer_plot_labels <- data_cancer_plot |>
  ...
  mutate(
    ...
    Rate = case_when(
      n() == 1 ~ Rate,
      max(Rate) - min(Rate) < Rate_range/2 & Rate == min(Rate) & Rate == max(Rate) & age_group == "20-49" ~  Rate - (Rate_range/2 -  (max(Rate) - min(Rate)))/2,
      max(Rate) - min(Rate) < Rate_range/2 & Rate == min(Rate) & Rate == max(Rate) & age_group == "50+" ~  Rate + (Rate_range/2 -  (max(Rate) - min(Rate)))/2,
      max(Rate) - min(Rate) < Rate_range/2 & Rate == min(Rate) ~ Rate - (Rate_range/2 -  (max(Rate) - min(Rate)))/2,
      max(Rate) - min(Rate) < Rate_range/2 & Rate == max(Rate) ~ Rate + (Rate_range/2 -  (max(Rate) - min(Rate)))/2,
      TRUE ~ Rate
    )
...
ggplot(data_cancer_plot, aes(x = year, y = Rate, fill = age_group, colour = age_group)) +
  # xlim(2001, 2027) +
  geom_point() +
  ...
  # scale_y_continuous(trans = "log") +
  ylab("Age Specific Rates") +
  xlab("Year") +
  labs(colour = "Age Group", fill = "Age Group", linetype = "Last Segment") +
  # scale_x_continuous(breaks = seq(min_year, max_year, by = 2)) +
  ggtitle("Trends in Cancer Incidence in England") +
  labs(subtitle = "Labels show APC for most recent trend, * indicates a significant trend") +
  facet_wrap(~ paste0(cancer_site, " - ", sex),  scales = "free_y", ncol = 4) 

ggsave(filename = paste0("../Output/Plots/Figure_1.png"), bg = "white", height = 10, width =10)

```

## Figure 1a - Trends in cancer incidence - ALTERNATIVE
...
plot <- ggplot(data_cancer, aes(x = year, y = Rate, colour = age_group)) +
  geom_point() +
  ...
  facet_wrap(~ facet_var,  scales = "free_y", ncol = 4)

print(plot)

ggsave(filename = paste0("../Output/Plots/Figure_1_Alt.png"), bg = "white", height = 10, width =10)
...
# Create Figure 1b
figure_1b <- full_joinpoint_table(data_joinpoint, data_joinpoint_AAPC, group_var = "cancer_site", table_var = "sex", stratification_vars = "age_group")

# Now edit, print and save
for(sex in names(figure_1b)){
  
  # Final changes
  figure_1b[[sex]] <- figure_1b[[sex]] |>
  ...
  # Save
  gtsave(figure_1b[[sex]], paste0("../Output/Plots/Figure_1b-", sex, ".html"))
...
## Figure 1c - Trends in cancer incidence APC table (selected cancers)
...
# Create Figure 1b
figure_1c <- full_joinpoint_table(data_joinpoint |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid")),
...
# Now edit, print and save
for(sex in names(figure_1c)){
  
  # Final changes
  figure_1c[[sex]] <- figure_1c[[sex]] |>
    cols_label(age_group = "Age Group") |>
    tab_header(paste0("Cancer Incidence Trends in ", sex))
  ...
  # Save
  gtsave(figure_1c[[sex]], paste0("../Output/Plots/Figure_1c-", sex, ".html"))
...
rf_trend_plot <- merge(data_rf |> mutate(level = if_else(level == "[30,Inf)" & variable == "fibre_consumption", "Below Guidelines", level),
                                         value = if_else(level == "Below Guidelines" & variable == "fibre_consumption", 1 - value, value)), 
                       rfdata_joinpoint_last,
                       by = c("sex", "age_group", "level", "variable"), all.x = T) |>
  mutate(
...
#plot
#ALCOHOL
alcoholtrends <- ggplot(rf_trend_plot %>% filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker"), aes(x = year, y = value, color = level)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  # geom_point(data = rf_trend_plot_invisible_points%>% filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker"), alpha = 0) +
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
  geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker"),
  ...
  scale_color_manual( #fixing the colors
    values = c(
      "Heavy Drinker" = "#f8333c",
      "Moderate Drinker" = "#2b9eb3",
      "Light Drinker" = "#fcab10" 
      # "Non-Drinker" = "#fcab10"
    )
  ) +
  scale_y_log10(breaks = scales::breaks_log()) +
...
#BMI
bmitrends <- ggplot(rf_trend_plot %>% filter(variable == "bmi")  |> filter(!level %in% c("Underweight", "Healthy Weight")), aes(x = year, y = value, color = level)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  # geom_point(data = rf_trend_plot_invisible_points %>% filter(variable == "bmi") |> filter(!level %in% c("Underweight", "Healthy Weight")), alpha = 0) +
   
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
  geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "bmi") |> filter(!level %in% c("Underweight", "Healthy Weight")),
    aes(label = APC_clean, group = level),
    nudge_x = 3,
    fill = "white", 
    size = 3,
...
#Smoking 
smokingtrends <- ggplot(rf_trend_plot %>% filter(variable == "smoking_status") |> mutate(level = if_else(level == "Previously", "Former", level)) |> filter(level != "Never"), aes(x = year, y = value, color = level)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  # geom_point(data = rf_trend_plot_invisible_points %>% filter(variable == "smoking_status") |> mutate(level = if_else(level == "Previously", "Former", level)) |> filter(level != "Never"), alpha = 0) +
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
    geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "smoking_status") |> mutate(level = if_else(level == "Previously", "Former", level)) |> filter(level != "Never"),
    aes(label = APC_clean, group = level),
    nudge_x = 3, 
    fill = "white", 
    size = 3,
...
#PHYSICAL ACTIVITY
PAtrends <- ggplot(rf_trend_plot %>% filter(variable %in% c("physical_activity", "physical_activity_old")) |> filter(level == "Below Recommendations") |> mutate(last_segment = if_else(variable == "physical_activity", FALSE, last_segment)), aes(x = year, y = value)) +
  ...
  # geom_point(data = rf_trend_plot_invisible_points %>% filter(variable %in% c("physical_activity", "physical_activity_old")) |> filter(level == "Below Recommendations"), alpha = 0) +
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
  geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable %in% c("physical_activity", "physical_activity_old")) |> filter(level == "Below Recommendations"),
...
diettrends <- ggplot(rf_trend_plot %>% filter(variable %in% c("redmeat_consumption_median", "processedmeat_consumption_median")), aes(x = year, y = value, color = variable)) +
  ...
  scale_color_manual( #fixing the colors 
    values = c(
      "redmeat_consumption_median" = "#fc9c9f", 
...
#Diet - Fibre
diettrends_fibre <- ggplot(rf_trend_plot %>% filter(variable == "fibre_consumption") |> filter(level == "Below Guidelines"), aes(x = year, y = value)) +
  geom_point(alpha = 0.6, size = 2, colour = "red") +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment), colour = "red") +
  # geom_point(data = rf_trend_plot_invisible_points %>% filter(variable == "fibre_consumption") |> filter(level == "Below Guidelines"), alpha = 0) +
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
   geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "fibre_consumption") |> filter(level == "Below Guidelines"),
    aes(label = APC_clean, group = variable),
    colour = "red",
    nudge_x = 3,
    fill = "white", 
  ...
  labs(
    title = "Fibre Deficiency in England",
    x = "Year",
    y = "Population Prevalence",
    color = "Exposure Category"
  ) +
...
## Figure 2a - Trends in risk factors - Alternate
...
for(p in names(jp_plots)){
  ...
  ggsave(jp_plots[[p]], 
       filename = paste0("../Output/Plots/Figure_2-", p, ".png"), 
       bg = "white", 
       height = 7, 
       width = 7)
...
## Figure 2b - Trends in Risk Factors Table
...
# Create table
figure2b_alt <- full_joinpoint_table(rfdata_joinpoint_complete, rfdata_joinpoint_complete_AAPC, group_var = "group_var", table_var = "sex", stratification_vars = "age_group")


# Now edit, print and save
for(sex in names(figure2b_alt)){
  
  # Final changes
  figure2b_alt[[sex]] <- figure2b_alt[[sex]] |>
    cols_label(age_group = "Age Group") |>
    ...
    row_group_order(groups = c(
      ...
      "Physical Activity - Below Recommendations",
      "Processed Meat Consumption - median",
      "Red Meat Consumption - median",
      "Fibre Consumption - Below Guidelines"
    ))
  
  # Print
  print(figure2b_alt[[sex]])
  
  # Save
  gtsave(figure2b_alt[[sex]], paste0("../Output/Plots/Figure_2b-", sex, ".html"))
...
for(var in unique(paste0(rf_imd_trend_plot$variable, "-", rf_imd_trend_plot$level))){
  ...
  ggsave(filename = paste0("../Output/Plots/Figure2IMDplot_", var, ".png"), plot = plot, width = 8, height = 6, bg = "white")
}
...
plot_year_summary <- data_years_all |>
  ...
  geom_col() +
  geom_segment(x = 14, xend = 14, y = -Inf, yend = 7.5, linetype ="dashed") +
  ...
  labs(
    ...
    subtitle = "Dashed line represents year used to calculate 2019 PAFs",
    x = "Year",
    y = "Variable"
...
## Figure 2e - Trends in Risk Factors overlayed with Colorectal Cancer Rate
...
rf_trend_plot_overlay <- rf_trend_plot |>
  filter(case_when(
    variable == "alcohol_amt" ~ level %in% c("Moderate Drinker", "Heavy Drinker"),
    variable == "bmi" ~ level %in% c("Overweight", "Obese"),
    variable == "physical_activity_old" ~ level == "Below Recommendations",
    variable == "fibre_consumption" ~ level == "Meets Guidelines",
  ...
  mutate(
    ...
    level = case_when(
      level == "Meets Guidelines" ~ "Below Guidelines",
    ...
    value = case_when(
      level == "Below Guidelines" ~ 1 - value,
      TRUE ~ value
    )
...
for(var in unique(rf_trend_plot_overlay$variable)){
  ...
  for(s in unique(rf_trend_plot_overlay$sex)){
    ...
    # Make plot
    plot_rf <- ggplot(data = rf_trend_plot_overlay_var, aes(x = year, y = value, colour = level, fill = level)) +
      geom_point() +
      geom_line() +
      scale_y_log10(breaks = scales::breaks_log(), limits = rf_trend_bounds) +
      theme_minimal() +
      theme(
      ...
      labs(
        ...
        x = "Year",
        y = if_else(var == "meat_consumption", "Daily Consumption (g)", "Prevalence"),
        colour = "",
        fill = "",
        linetype = ""
      ) +
    ...
    # Make plot
    plot_cancer <- ggplot(data = rf_overlay_colorectal |> filter(sex == s), aes(x = year, y = value, colour = level, fill = level)) +
      geom_point() +
      geom_line() +
      scale_y_log10(breaks = scales::breaks_log(), limits = cancer_trend_bounds) +
      theme_minimal() +
      theme(
        axis.text.y = rotatedAxisElementText(90,'y'),
    ...
    plot <- plot_grid(plot_cancer, plot_rf, nrow = 2)

    print(plot)

    ggsave(filename = paste0("../Output/Plots/Figure2e_", var, "_", s, ".png"), plot = plot, width = 6, height = 6, bg = "white")
...
# Also output a plot for Endometrial and BMI in women

# Extract the colorectal cancer rates
rf_overlay_endo <- data_cancer |>
  filter(cancer_site == "Endometrium") |>
  filter(age_group == "20-49") |>
  select(year, value = Rate, sex) |>
  mutate(level = "Endometrial Cancer Rate",
         variable = "Cancer Rate")
...
plot_rf_legend <- get_legend(plot_rf)
plot_rf <- plot_rf +
  theme(legend.position = "none")

# Add blank to plot_rf_legend
plot_rf_legend <- plot_grid(get_legend(plot_rf), plot_rf_legend, ncol = 1)

# Make plot
plot_cancer <- ggplot(data = rf_overlay_endo |> filter(sex == s), aes(x = year, y = value, colour = level, fill = level)) +
  geom_point() +
  geom_line() +
  scale_y_log10(breaks = scales::breaks_log()) +
  theme_minimal() +
  ...
  labs(
    title = paste0("Trends of Endometrial Cancer and ", var, " in 20-49 ", s),
    x = "Year",
    y = "Cancer Rate",
    colour = "",
    fill = "",
...
plot <- plot_grid(plot_cancer, plot_rf, nrow = 2)
plot <- plot_grid(plot, plot_rf_legend, nrow = 1, rel_widths = c(1, 0.3))

print(plot)

ggsave(filename = paste0("../Output/Plots/Figure2e_bmi_endomtrium.png"), plot = plot, width = 6, height = 6, bg = "white")
...
for(s in unique(rf_trend_plot_overlay$sex)){
  ...
  plot_cancer <- ggplot(data = rf_overlay_colorectal |> filter(sex == s), aes(x = year, y = value, colour = level, fill = level)) +
        ...
        scale_x_continuous(breaks = seq(2005, 2030, 5), limits = c(2005, 2030), position= "top") +
        scale_y_continuous(sec.axis = sec_axis(~ ., name = "Colorectal Cancer Rate", 
                                            labels = NULL))
  ...
  for(var in unique(rf_trend_plot_overlay$variable)){
    ...
    plot_rf <- ggplot(data = rf_trend_plot_overlay_var, aes(x = year, y = value, colour = level, fill = level)) +
      ...
      scale_x_continuous(breaks = seq(1995, 2020, 5), limits = c(1995, 2020)) +
      scale_y_continuous(sec.axis = sec_axis(~ ., name = var, 
                                          labels = NULL))
    ...
    if(which(unique(rf_trend_plot_overlay$variable) == var) == length(unique(rf_trend_plot_overlay$variable))){
      plot_rf <- plot_rf +
        labs(x = "Year")
    }
  ...
  plot_legend <- plot_grid(plotlist = legend_list, ncol = 1, rel_heights = c(1.05, rep(1, length(plot_list)-2), 1.05))
  plot <- plot_grid(plot, plot_legend, nrow = 1, rel_widths = c(1, 0.35))
  
  print(plot)
  
  ggsave(filename = paste0("../Output/Plots/Figure2e_Alt_", s, ".png"), plot = plot, width = 8, height = 14, bg = "white")
...
### Calculate CIs for Aggregate PAFs
...
data_paf_agg_ci <- data_paf_comparison_agg |>
  ...
  mutate(
    Cancer_sites = if_else(Cancer_sites == "Multiple Myeloma", "MultipleMyeloma", Cancer_sites)
  )
...
ggsave("../Output/Plots/Figure_3_PAF.png", plot = data_attr_bar_paf, bg = "white", height = 6, width = 14)
...
## Figure 4c - Current PAFs (2019) - Side Bar chart
...
data_paf_spec_ci <- data_paf_comparison_spec |>
  ...
  mutate(
    ...
    cancer_site = if_else(cancer_site == "Multiple Myeloma", "MultipleMyeloma", cancer_site)
  )
...
data_attr_cases_bar_ordering  <- data_attr_cases_bar |>
  mutate(cancer_site = gsub("\\s\\-.*", "", facet_var)) |>
  filter(variable == "BMI") |>
  filter(!is.na(cases_count)) |>
  group_by(cancer_site) |>
...
#     
#   ) |>
#   select(-PIF_past, -PIF_future, -PIF_past_future) |>
...
#     
#     
#     PIF = 100*round(PIF_past_future, digits = 3),
#     
#     # PIF = if_else(is.na(PIF), "-", as.character(PIF))
#     
#     PIF = case_when(
...
#     variable ~ px(200)
#   ) |>
#   tab_header("PIF Values from Past to Future*") |>
#   tab_footnote("*Furthest back datapoint available up to furthest forward datapoint available")
# 
# 
# data_pif_gt_future
# 
# gtsave(data_pif_gt_future, "../Output/Plots/Figure_6-PIF_Past_Future.html")
  

```

## Figure 6e - PIF (BMI only)
...
data_pif_bmi_gt <- data_pif_bmi |>
  ...
  tab_spanner(
    label = md("*Cancer Incidence: 2005 - 2019*"),
    columns = starts_with("PIF_past")
  ) |>
  tab_spanner(
    label = md("*Cancer Incidence: 2019 - 2029*"),
...
gtsave(data_pif_bmi_gt, "../Output/Plots/Figure_6-PIF_BMI.html")


```

## Figure 6f - PIF (BMI only) HEATMAP
...
data_pif_bmi <- PIF |>
  ...
  mutate(
    ...
    value = case_when(
      is.na(value) ~ -999, # Special error code so we can handle it specially
      value < 0 ~ NA,
      TRUE ~ round(100*value, digits = 2)
    ),
...
data_pif_bmi_gt <- data_pif_bmi |>
  ...
  data_color(
    columns = names(data_pif_bmi)[-1],
    colors = scales::col_numeric(
      palette = c("white", "red"),
      domain = c(0, max(as.numeric(unlist(data_pif_bmi[,-1])), na.rm = TRUE))
    ),
    apply_to = "fill"  
  ) |>
  fmt(
    columns = names(data_pif_bmi)[-1],
    rows = cancer_site %in% c("Breast", "Endometrium"),
    fns = function(x){if_else(x == -999, "-", as.character(x))}
  ) |>
  # tab_style(
  #   style = cell_fill(color = "white"),  # Keep "-" cells white
...
# Loop through columns making sure that "-" gets a white background
for(col in names(data_pif_bmi[-1])){
  data_pif_bmi_gt <- data_pif_bmi_gt |>
    tab_style(
...
gtsave(data_pif_bmi_gt, "../Output/Plots/Figure_6-PIF_BMI_Heatmap.html")
...
data_pif_bmi <- merge(data_pif_bmi, data_paf_pif_ci, by.x = c("cancer_site", "age_group", "sex", "name"), by.y = c("Cancer_sites", "age_group", "sex", "name"))
...
ggsave("../Output/Plots/Figure_6-PIF_BMI_Barchart.png", plot = plot, bg = "white", height = 8, width = 8)
...
### Childhood BMI (+ Colorectal Cancer Trends + Obesity Trends)

```{r}

# Get childhood obesity data
data_childhood_bmi_clean <- data_childhood_bmi |>
  ...
  mutate(value = value/100,
         age_group = "2-15",
         sex = if_else(sex == "Boys", "Men", "Women"))

# Get adult obesity data for the same plot
data_adult_bmi_clean <- rf_trend_plot |>
  filter(level == "Obese") |>
  filter(age_group == "20-49") |>
  select(year, age_group, sex, level, value)
...
rf_overlay_colorectal <- data_cancer |>
  filter(cancer_site == "Colorectum") |>
  filter(age_group == "20-49") |>
  select(year, value = Rate, sex, age_group) |>
  mutate(level = "Colorectal Cancer Rate",
         variable = "Cancer Rate") |>
  select(year, age_group, sex, level, value)

# Combine into one dataset
data_colorectal_bmi_plot <- rbind(data_childhood_bmi_clean, data_adult_bmi_clean) |>
  mutate(level = paste0(level, " - ", age_group))

# Need to conduct rescaling
cancer_range <- range(rf_overlay_colorectal$value)

data_colorectal_bmi_plot <- data_colorectal_bmi_plot |>
  mutate(
    value_scaled = (diff(cancer_range)/diff(range(value)))*(value - min(value)) + cancer_range[1]
  )

rf_overlay_colorectal <- rf_overlay_colorectal |>
  mutate(
    value_scaled = value
  )

rf_overlay_plot <- rbind(rf_overlay_colorectal, data_colorectal_bmi_plot) |>
  mutate(level = factor(level, levels = unique(level)))

# Make plot
plot <- ggplot(data = rf_overlay_plot, aes(x = year, y = value_scaled, colour = level, fill = level)) +
  geom_point() +
  geom_line() +
  scale_y_continuous(
    sec.axis = sec_axis(~ (.-cancer_range[1])/(diff(cancer_range)/diff(range(rf_trend_plot_overlay_var$value))) + min(rf_trend_plot_overlay_var$value),
                        name = "Prevelance (%)")
  ) +
  theme_minimal() +
  theme(
  ...
  labs(
    title = paste0("Trends of Colorectal Cancer and Obesity"),
    x = "Year",
    y = "Cancer Rate",
    colour = "",
    fill = "",
...
ggsave(filename = paste0("../Output/Plots/Figure2e_childhood_obesity.png"), plot = plot, width = 12, height = 6, bg = "white")
...
# Removing 2020 as COVID affected cases
ggplot(data_childhoodbmi_plot, aes(x = year, y = value, fill = level, colour = level)) +
  xlim(1995, 2021.5) +
  geom_point() +
  geom_point(data = data_childhoodbmi_invisible_points, alpha = 0) +
  geom_line(aes(linetype = last_segment)) +
  geom_label_repel(
    data = data_childhoodbmi_plot_labels,
    aes(label = APC_clean),
    nudge_x = 3,
    fill = "white",
    size = 3,
  ...
  labs(subtitle = "Labels show APC for most recent trend, * indicates a significant trend") +
  facet_grid(rows = vars(sex), cols = vars(age_group), scales = "free_y") 
...
![](images/inc_asr_gb_smoking_prev_i19.png)

### Alcohol

![](images/total-alcohol-consumption-per-capita-litres-of-pure-alcohol.png)

### Red and Processed Meat

```{r}

data_food_trends <- readODS::read_ods("../Data/UKHHcons_17Oct24.ods", sheet = "Household_quantity")

colnames(data_food_trends) <- data_food_trends[4,]

# Selecting only carcas meat section for simplicity
data_food_trends <- data_food_trends[39,]

# Get index of first year, last year and filter data to that
first_year <- which(colnames(data_food_trends) == "1974")
last_year <- which(colnames(data_food_trends) == "202223")

data_food_trends <- data_food_trends[,first_year:last_year] |>
  mutate(across(where(is.character), as.numeric)) |>
  pivot_longer(cols = everything(), names_to = "Year")  |>
  mutate(
    
    Year = case_when(
      grepl("\\-", Year) ~ gsub("\\-.*", "", Year),
      nchar(Year) == 6 ~ paste0(substr(Year, 1, 2), substr(Year, 5, 6)),
      TRUE ~ Year
    ),
...
plot <- ggplot(data_food_trends, aes(x = Year, y = value)) +
  geom_point() +
  geom_line() +
  theme_minimal() +
  labs(
    title = "Carcass Meat Consumption UK",
    y = "Carcass Meat Purchased (g/week)"
  )
...
## Historical Colorectal Risk Factor Trends

### Function

```{r}

plot_with_colorectal <- function(data, data_colorectal, sec.axis.name = NULL){
  
  # Need to conduct rescaling
  cancer_range <- range(data_colorectal$value)
  ...
  data_colorectal <- data_colorectal |>
    mutate(
      value_scaled = value
    )
  
  data_plot <- rbind(data_colorectal, data) |>
    mutate(variable = factor(variable, levels = unique(variable)))
  
  # Make plot
  plot <- ggplot(data = data_plot, aes(x = year, y = value_scaled, colour = variable, fill = variable)) +
    geom_point() +
    geom_line() +
    scale_y_continuous(
      sec.axis = sec_axis(~ (.-cancer_range[1])/(diff(cancer_range)/diff(range(data$value))) + min(data$value),
                          name = sec.axis.name)
    ) +
    theme_minimal() +
    theme(
...
### Long term colorectal cancer rate trends

```{r}

data_lt_colorectal <- read.csv("../Data/dataset-asr-inc-males-and-females-age-20-49-1943-2020-colorectum.csv") |>
  mutate(
    sex = if_else(Sex == 1, "Men", "Women"),
    variable = paste0("Colorectal Cancer - ", sex)
  ) |>
  select(year = Year, variable, value = ASR..World.)
...
data_lt_alcohol <- read.csv("../Data/per-capita-alcohol-1890.csv") |>
  filter(Year >= 1960) |>
  select(year = Year, value = Alcohol.consumption.since.1890..Alexander...Holmes..2017.) |>
  mutate(variable = "Alcohol Consumption")

plot <- plot_with_colorectal(data_lt_alcohol, data_lt_colorectal, sec.axis.name = "Litres/Year")
...
data_food_trends_lt <- data_food_trends |>
  select(year = Year, value) |>
  mutate(variable = "Carcass Meat Consumption")

plot <- plot_with_colorectal(data_food_trends_lt, data_lt_colorectal)
...
![](images/trend-chart-breast-screening.png)

#### Colorectal

![](images/trend-chart-bowel-screening.png)

#### Cervical

![](images/trend-chart-cervical-25-49.png)

![](images/trend-chart-cervical-50-64.png)
...
pifobesescenario_table <- obesescenario %>%
  ...
  tab_header(
    title = md("BMI PIFs for Colorectal Cancer with scenario RR")
  )
