...
gtsave(apc_table, "../Output/Plots/Figure_1c-Selected_Cancer_APC.html")
...
rf_trend_plot <- merge(data_rf |> mutate(level = if_else(level == "[30,Inf)" & variable == "fibre_consumption", "Below Guidelines", level),
                                         value = if_else(level == "Below Guidelines" & variable == "fibre_consumption", 1 - value, value)), rfdata_joinpoint_last, by = c("sex", "age_group", "level", "variable"), all.x = T) |>
  mutate(
...
gtsave(apc_table, "../Output/Plots/Figure_2b-RF_APC.html")
...
# Calculating aggregate PAFs from the sampled PAFs
data_paf_comparison_pif <- data_paf_comparison |>
  filter(variable == "bmi") |>
  group_by(age_group, sex, year, Cancer_sites) |>
  mutate(iteration = row_number()) |>
  ungroup() |>
  group_by(age_group, sex, iteration, Cancer_sites) |>
  arrange(year) |>
...
# Now calculating confidence intervals for the aggregate PAFs
data_paf_pif_ci <- data_paf_comparison_pif |>
  pivot_longer(cols = c(PIF_past, PIF_future)) |>
...
plot <- ggplot(data_pif_bmi, aes(x = cancer_site, y = value, fill = age_group)) +
  geom_bar(position = "dodge", stat = "identity", colour = "black") +
