...
ggplot(data_cancer_plot, aes(x = year, y = Rate, fill = sex, colour = sex)) +
  ...
  labs(subtitle = "Labels show APC for most recent trend, * indicates a significant trend") +
  facet_wrap(~ paste0(cancer_site, " - ", age_group),  scales = "free_y", ncol = 4) 
...
for(var in unique(rf_trend_plot_overlay$variable)){
  ...
  rf_trend_plot_overlay_var <- rf_trend_plot_overlay_var |>
    mutate(
  ...
  rf_overlay_plot <- rbind(rf_overlay_colorectal, rf_trend_plot_overlay_var) |>
    mutate(
      
      level = case_when(
        level == "Colorectal Cancer Rate" ~ level,
  ...
  plot <- ggplot(data = rf_overlay_plot, aes(x = year, y = value_scaled, colour = level, fill = level)) +
    ...
    scale_y_continuous(
      sec.axis = sec_axis(~ (.-cancer_range[1])/(diff(cancer_range)/diff(range(rf_trend_plot_overlay_var$value))) + min(rf_trend_plot_overlay_var$value),
                          name = if_else(var == "Meat Consumption (Median)", "Median Consumption Meat (g/day)", "Prevelance (%)"))
    ) +
    theme_minimal() +
    theme(
  ...
  ggsave(filename = paste0("../Output/Plots/Figure2e_", var, ".png"), plot = plot, width = 12, height = 6, bg = "white")
