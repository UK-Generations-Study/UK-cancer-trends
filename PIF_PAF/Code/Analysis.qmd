...
data_attr_bar_MEN <- ggplot(data_attr_cases_bar %>% filter(sex == "Men"), aes(x = cancer_site)) +
  ...
  labs(
    title = "Men: PAF and Case Count in England",
    x = "Cancer Site"
  ) +
...
data_attr_bar_WOMEN <- ggplot(data_attr_cases_bar %>% filter(sex == "Women"), aes(x = cancer_site)) +
  ...
  scale_fill_manual(
    values = c("PAF" = "#2b9eb3", "Case Count" = "#f8333c"),  # Set custom colors
    name = "Key"  # Add a legend title
  ) +
  labs(
    title = "Women: PAF and Case Count in England",
    x = "Cancer Site"
  ) +
  facet_wrap(~ age_group, scales = "free_x") +
  theme_minimal() +
...
data_attr_cases_bar <- data_paf_long_individual %>% 
  ...
  filter(
    year == 2019 | (year == 2018 & all(PAF[year == 2019] == 0)) 
  ) %>%
  select(
    -count, - Rate
  )
...
generate_plots <- function(data) {
  # Extract unique combinations of age_group, sex, and variable
   combinations <- data %>%
    distinct(age_group, sex, variable)
  ...
  for (i in seq_len(nrow(combinations))) {
    ...
    # Generate the plot
        plot <- ggplot(filtered_data, aes(x = cancer_site)) +
      geom_bar(
        aes(y = PAF, fill = "PAF"), 
        stat = "identity", 
        width = 0.8,
      ...
      geom_bar(
        aes(y = cases_count / 100, fill = "Attributable Cases"),  # Ensure this uses a proper aesthetic
        stat = "identity", 
        width = 0.3,
        alpha = 1
      ) +
      ...
      scale_fill_manual(
        values = c("PAF" = "#2b9eb3", "Attributable Cases" = "#f8333c"),  # Ensure color is assigned correctly
        name = "Key"
      ) +
      labs(
        title = paste(current_variable, "(", current_sex, current_age_group, ") : PAF and Case Count in England"),
        x = "Cancer Site"
      ) +
      facet_wrap(~ sex + variable, scales = "free_y") +  # Stack plots by sex and variable
      theme_minimal() +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "top"
