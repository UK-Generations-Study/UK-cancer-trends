...
data_attr_bar_MEN <- ggplot(data_attr_cases_bar %>% filter(sex == "Men"), aes(x = cancer_site)) +
  ...
  geom_bar(
    aes(y = cases_count/100, fill = "Case Count"),
    stat = "identity", 
...
######START HERE AND TRYNG TO LOOP TRHOUGH THE DATA FRAME 
pifscenarios = dataframe
# Loop through
PIF1 <- ((exposurerange1(max) - exposurerange1)(RR -1))/(1+(exposurerange1(max)*(RR-1))
PIF2 <- ((exposurerange2(max) - exposurerange2)(RR -1))/(1+(exposurerange2(max)*(RR-1))
PIF3 <- ((exposurerange3(max) - exposurerange3)(RR -1))/(1+(exposurerange3(max)*(RR-1))
PIF4 <- ((exposurerange4(max) - exposurerange4)(RR -1))/(1+(exposurerange4(max)*(RR-1))
# Plot the PAF against the prevalence
pifplotlow <- ggplot(data.frame(exposure = exposurerange, PIF = PIFlow), aes(x = exposure, y = PIF)) +
  geom_line(color = "blue", size = 1) +
  labs(title = "RR = 1.2",
       x = "RF Prevalence",
       y = "PIF") +
...
#### RR Scenario Example-will not be used
