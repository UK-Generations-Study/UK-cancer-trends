...
#   Cancer Incidence
data_joinpoint <- read.delim("../Data/Joinpoint_Results/Cancerrates/joinpoint_cancerrates.Export.APC.txt", sep = "\t")
#   Risk Factors
...
### Ensuring the Joinpoint input is up to data
```{r}
...
for (var in unique_level) {
  plot <- ggplot(data_rf_imd_clean %>% filter(complete == var), aes(x = year, y = value, color = as.factor(imd), group = imd)) +
    geom_line(size = 1) +
    labs(
