...
rf_trend_plot <- merge(data_rf, rfdata_joinpoint_last, by = c("sex", "age_group", "level", "variable"), all.x = T) |>
  mutate(
...
#BMI
bmitrends <- ggplot(rf_trend_plot %>% filter(variable == "bmi"), aes(x = year, y = value, color = level)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  geom_point(data = rf_trend_plot_invisible_points, alpha = 0) +
...
#Smoking 
smokingtrends <- ggplot(rf_trend_plot %>% filter(variable == "smoking_status"), aes(x = year, y = value, color = level)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  geom_point(data = rf_trend_plot_invisible_points, alpha = 0) +
  ...
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
    geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable == "smoking_status"),
    aes(label = APC_clean, group = level),
    nudge_x = 3, 
    fill = "white", 
    size = 3,
...
#Diet
diettrends <- ggplot(rf_trend_plot %>% filter(variable %in% c("redmeat_consumption_median", "processedmeat_consumption_median", "fibre_consumption_mean")), aes(x = year, y = value, color = variable)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter plot for the original data
  geom_line(aes(linetype = last_segment)) +
  geom_point(data = rf_trend_plot_invisible_points, alpha = 0) +
  ...
  facet_grid(sex ~ age_group) +  # grid by both age_group and sex
   geom_label_repel(
    data = rf_trend_plot_labels %>% filter(variable %in% c("redmeat_consumption_median", "processedmeat_consumption_median", "fibre_consumption_mean")),
    aes(label = APC_clean, group = variable),
    nudge_x = 3,
    fill = "white", 
    size = 3,
  ...
  scale_color_manual( #fixing the colors 
    values = c(
      ...
      "processedmeat_consumption_median" = "#fcab10",
      "fibre_consumption_mean" = "#f8333c"
    )
  ) +
      scale_x_continuous(
    breaks = seq(1995, 2019, by = 3),  # Define the x-axis breaks
