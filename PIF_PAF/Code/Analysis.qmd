...
The cancer sites that were excluded from the study were: Leukaemia, Prostate, Hodgkin Lymphoma, Brain and CNS, Melanoma of skin, and Cervix.

**Methodological plan:**

Relative risk and risk factor exposure prevalences will be pulled from peer reviewed meta-analyses and open-source health datasets, respectively.

Data sources include National Diet and Nutrition Survey, Health Survey England, IARC GLOBOCAN, etc.

Cancer incidence and risk factor trends will be examined from 2000 to present day, or as far back as data are available. These trends will be shown overall, and by age split (50-year cutoff).
...
1.  Alcohol data stops at 2011, data has been inferred from the General Household Survey 2005 but the alcohol question may not be entirely consistent. This is mostly to do with the classification between non and light drinkers.  
    ...
    -   Could project backwards from HSE values 
...
3.  Oral contraceptive data source potentially non-existent 

4.  Would need to ask for data access for OC and HRT from previous papers written using NHS financial data. 

5.  Montse has suggested industry data for dietary factors, purely for graphic purposes. These should be found where possible. 
...
for(var in unique(data_rf_trends$variable)){
  ...
  if(var == "diet"){
    plot <- plot +
      labs(caption = "*Red and Processed Meat uses the median, Fibre uses the mean")
  }
...
for(var in unique(data_complete_paf_plot$variable_label)){
  ...
  # Make forest plot
  pdf(file=paste0("../Output/Plots/Figure 4: ", var, ".pdf"), width = 15, height = (2/5)*(3 + rows))
  print(plot)
  dev.off()
  
  cat(sprintf("- [%s](%s)\n", paste0("**Figure 4: ", var, "**"), paste0("../Output/Plots/Figure 4: ", var, ".pdf")))
...
data_pif_gt_past <- data_pif |>
  ...
  tab_header("PIF Values from Past to Present")

data_pif_gt_past

gtsave(data_pif_gt_past, "../Output/Plots/Figure 5 - PIF Past.html")
...
data_pif_gt_future <- data_pif |>
  ...
  tab_header("PIF Values from Present to Future")


data_pif_gt_future

gtsave(data_pif_gt_future, "../Output/Plots/Figure 5 - PIF Future.html")
