...
plot <- ggplot(data_cancer_facet, aes(x = year, y = Rate, colour = age_group)) +
  ...
  labs(caption = "(Refer to Supplementary Table 4)")
...
for(p in names(jp_plots)){
  ...
  current_caption <- jp_plots[[p]]$labels$caption
  
  if(is.null(current_caption)){
    jp_plots[[p]] <- jp_plots[[p]] +
      labs(caption = "(Refer to Supplementary Table 5)")
  } else {
    jp_plots[[p]] <- jp_plots[[p]] +
      labs(caption = md(paste0(current_caption, "\n(Refer to Supplementary Table 5)")))
  }
...
data_attr_bar_paf <- ggplot(data_attr_cases_bar_alt, aes(x = cancer_site, y = PAF, fill = age_group)) +
  ...
  labs(
    title = "Population Attributable Fractions: 2019",
    fill = "Age Group",
    x = "Cancer Site",
    y = "PAF (%)"
  )+
...
ggsave("../Output/Final_Plots/Figure_3.png", plot = data_attr_bar_paf, bg = "white", height = 6, width = 14)
...
p <- ggplot(BMI_paf_cases_u50, aes(x = year, y = value, colour = rate_type)) +
    ...
    labs(
      ...
      y = "Rate",
      title = paste("BMI Attributable Rates in Young Adults"),
      colour = "Rate Type"
    ) +
    theme_minimal() +
    geom_rect(
    ...
    theme(strip.text = element_text(face = "bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(caption = "(Refer to Supplementary Table 6)")

print(p)

# Save the plot as a PNG file
ggsave(paste0("../Output/Final_Plots/Figure_4.png"), plot = p, width = 10, height = 10)
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  grid.text("^ - AAPC for 20-49 is significantly greater than 0 the AAPC for 50+ is not significantly greater than 0 or does not overlap with the confidence interval of the AAPC for 25-49", x = 0.3605, y = 0.008, gp = gpar(fontsize = 8))
  
  # Save plot
  pdf(file=paste0("../Output/Final_Plots/Supplementary_Figure_2", letters[which(unique(data_joinpoint_AAPC$sex == s))], ".pdf"), width = 10, height = (4/15)*(3 + rows))
  print(plot)
  grid.text("* - AAPC for 20-49 is significantly greater than 0", x = 0.125, y = 0.03, gp = gpar(fontsize = 8))
  grid.text("^ - AAPC for 20-49 is significantly greater than 0 and the AAPC for 50+ is not significantly greater than 0 or does not overlap with the confidence interval of the AAPC for 25-49", x = 0.436, y = 0.01, gp = gpar(fontsize = 8))
  dev.off()
...
for(var in unique(paste0(rf_imd_trend_plot$variable, "-", rf_imd_trend_plot$level))){
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_3", letters[which(unique(paste0(rf_imd_trend_plot$variable, "-", rf_imd_trend_plot$level)) == var)], ".png"), plot = plot, width = 8, height = 6, bg = "white")
}
...
for (var in unique (data_attr_cases_bar$variable)) {
  ...
  data_attr_bar_paf <- ggplot(subset_data, aes(x = cancer_site, y = PAF, fill = age_group)) +
    ...
    labs(
      title = paste(var, " Population Attributable Fractions: 2019"),
      fill = "Age Group",
      x = "Cancer Site",
      y = "PAF (%)"
    )+
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_4", letters[which(unique (data_attr_cases_bar$variable) == var)], ".png"), plot = data_attr_bar_paf, width = 8, height = 6, bg = "white")
  
  if(var == "Body Mass Intake (BMI)"){
    ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_4", letters[which(unique (data_attr_cases_bar$variable) == var)], "_alt.png"), plot = data_attr_bar_paf, height = 6, width = 14, bg = "white")
  }
...
p <- ggplot(BMI_paf_cases_oe50, aes(x = year, y = value, colour = rate_type)) +
    ...
    labs(
      x = "Year",  # Corrected axis label
      y = "Rate",
      title = paste("BMI Attributable Rates for Older Adults"),
      colour = "Rate Type"
    ) +
    theme_minimal() +
    geom_rect(
    ...
    theme(strip.text = element_text(face = "bold"), 
        axis.text.x = element_text(angle = 45, hjust = 1)) +
    labs(caption = "Refer to Supplementary Table 6 for detailed JoinPoint results")

print(p)

# Save the plot as a PNG file
ggsave(paste0("../Output/Final_Plots/Supplementary_Figure_5.png"), plot = p, width = 10, height = 10)
...
ggsave("../Output/Final_Plots/Supplementary_Figure_6.png", plot = pafplotall, bg = "white", height = 6, width = 10)
