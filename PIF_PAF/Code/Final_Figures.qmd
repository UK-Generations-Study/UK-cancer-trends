...
for(s in unique(data_joinpoint_model$sex)){
  ...
  invisible_points <- data_cancer |>
    ...
    summarise(max = max(Rate), min = min(Rate)) |>
    ungroup() |>
    arrange(desc(max)) |>
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Figure_1", letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".png"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_1",  letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".svg"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
...
ggsave("../Output/Final_Plots/Figure_3a.png", plot = data_attr_bar_paf, bg = "white", height = 4, width = 9)
ggsave("../Output/Final_Plots_SVG/Figure_3a.svg", plot = data_attr_bar_paf, bg = "white", height = 4, width = 9)

```

### Figure 4 - BMI Attributable Trends
```{r, fig.height=14, fig.width=12, warning=F}
...
for(sex in c("Women", "Men")){
  ...
  # gtsave(supp_table_4[[sex]], paste0("../Output/Final_Plots/Supplementary_Table_4", letters[which(c("Women", "Men") == sex)], ".png"))
...
for(sex in c("Women", "Men")){
  ...
  # gtsave(supp_table_5[[sex]], paste0("../Output/Final_Plots/Supplementary_Table_5", letters[which(c("Women", "Men") == sex)], ".png"))
...
for(group in plot_order){
  ...
  # gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots/Supplementary_Table_6", letters[which(plot_order == group)], ".png"))
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  data_joinpoint_AAPC_sf2 <- data_joinpoint_AAPC |>
    ...
    mutate(
      cancer_site = if_else(p_value_one_sided < 0.05, paste0(cancer_site[2], "^"), cancer_site[2])
    )
  ...
  grid.text("^ - AAPC for 20-49 is significantly greater than 0 and the AAPC for 50+", x = 0.195, y = 0.008, gp = gpar(fontsize = 8))
  ...
  grid.text("^ - AAPC for 20-49 is significantly greater than 0 and the AAPC for 50+", x = 0.195, y = 0.01, gp = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"))
  dev.off()
...
plot_order <- c("Cigarette Smoking", "Alcohol Consumption", "Body Mass Intake (BMI)", "Physical Inactivity", "Fibre Intake Deficiency", "Red Meat Consumption", "Processed Meat Consumption")
...
for (var in plot_order) {
  ...
  if(var == "Body Mass Intake (BMI)"){
    ...
    data_attr_bar_paf <- data_attr_bar_paf +
    labs(
      title = paste0("Population Attributable Fractions 2019: ", var),
    )
    
    ggsave(filename = paste0("../Output/Final_Plots/Figure_3b.png"), plot = data_attr_bar_paf, height = 4, width = 9, bg = "white")
    ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_3b.svg"), plot = data_attr_bar_paf, height = 4, width = 9, bg = "white")
  }
...
for(s in unique(data_bmi_esp$sex)){
  ...
  BMI_paf_cases_oe50_invisible <- BMI_paf_cases_oe50 |>
    ...
    mutate(
      ...
      value = case_when(
        cancer_site == "Oral" & name == "min_value" ~ 5,
        # cancer_site == "Breast" & name == "min_value" ~ 25,
        TRUE ~ value
  ...
  p <- ggplot(BMI_paf_cases_oe50, aes(x = year, y = value, colour = rate_type)) +
      geom_line() +
      geom_point() +
      geom_point(data = BMI_paf_cases_oe50_invisible, alpha = 0) +
      # facet_manual(~ facet_var, facet_design, respect = T, scales = "free_y") +
      facet_wrap(~ facet_var, scales = "free_y", ncol = 3) +
      scale_y_log10(breaks = scales::breaks_log(n = 8),
                    labels = scales::label_number(accuracy = 0.01)) +
...
### Supplementary Figure 7 - Colorectal cancer and risk factor plotting
...
for(s in c("Women", "Men")){
  ...
  plot_cancer <- ggplot(data = rf_overlay_colorectal |> filter(sex == s), aes(x = year, y = value, colour = level)) +
        ...
        labs(
          ...
          x = "Year",
          y = "Age Standardised Incidence Rates",
          colour = "",
          fill = "",
          linetype = ""
        ) +
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_7", letters[which(c("Women", "Men") == s)], ".png"), plot = plot, width = 9, height = 14, bg = "white")
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_7", letters[which(c("Women", "Men") == s)], ".svg"), plot = plot, width = 9, height = 14, bg = "white")
...
for(s in c("Women", "Men")){
  ...
  # Now loop through variables
  for(var in c("BMI (2-15 years)", "BMI (20-49 years)", "Alcohol Consumption", "Physical Inactivity", "Fibre Intake Deficiency", "Meat Consumption")){
    ...
    if(var %in% c("BMI (20-49 years)", "BMI (2-15 years)")){
      ...
      plot_rf <- plot_rf +
        ...
        scale_y_log10(breaks = scales::breaks_log(), limits = rf_trend_plot_overlay |>
      filter(variable %in% c("BMI (2-15 years)", "BMI (20-49 years)")) |>
      pull(value) |>
      range(), sec.axis = sec_axis(~ ., name = var, labels = NULL))
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_7", letters[which(c("Women", "Men") == s)], "_temp.png"), plot = plot, width = 9, height = 14, bg = "white")
  # ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_7_TEMP", letters[which(c("Women", "Men") == s)], "_temp.svg"), plot = plot, width = 9, height = 14, bg = "white")
