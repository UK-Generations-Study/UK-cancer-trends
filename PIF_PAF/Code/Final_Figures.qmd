...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  # Save plot
  pdf(file=paste0("../Output/Final_Plots/Supplementary_Figure_1", letters[which(unique(data_joinpoint_AAPC$sex == s))], ".pdf"), width = 10, height = (4/15)*(3 + rows))
  print(plot)
  grid.text("* - AAPC for 20-49 is significantly greater than 0", x = 0.125, y = 0.03, gp = gpar(fontsize = 8))
  grid.text("^ - AAPC for 20-49 is significantly greater than 0 and the AAPC for 50+ is not significantly greater than 0 or does not overlap with the confidence interval of the AAPC for 20-49", x = 0.436, y = 0.01, gp = gpar(fontsize = 8))
  dev.off()
...
for(s in c("Women", "Men")){
  ...
  plot <- ggdraw() +
    ...
    draw_label(paste0("Trends in Colorectal Cancer and Risk Factors in Young ", s), fontface = 'bold', 
               x = 0, y = 0.99,  
               hjust = -0.05, vjust = 1,  
               size = 14)
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_7", letters[which(c("Women", "Men") == s)], ".png"), plot = plot, width = 8, height = 14, bg = "white")
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_7", letters[which(c("Women", "Men") == s)], ".svg"), plot = plot, width = 8, height = 14, bg = "white")
