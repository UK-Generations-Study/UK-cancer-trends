...
facet_design <- matrix(c(1:length(facet_design), as.vector(rbind(length(facet_design) + (1:length(facet_design_extra)), NA)), NA, NA), ncol = 4, byrow = T)
...
plot <- ggplot(data_cancer_facet, aes(x = year, y = Rate, colour = age_group)) +
  ...
  ylab("Age Specific Incidence Rates per 100,000 (log scale)") +
  xlab("Year") +
...
ggsave(filename = paste0("../Output/Final_Plots/Figure_1.png"), bg = "white", height = 12, width =12)
ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_1.svg"), bg = "white", height = 12, width =12)
...
# Need to manual construct structure for facet
facet_design <- data_cancer |>
  group_by(cancer_site) |>
  mutate(facet_var = paste0(cancer_site, " - ", sex),
            n = n()) |>
...
p <- ggplot(BMI_paf_cases_u50, aes(x = year, y = value, colour = rate_type)) +
    geom_line() +
    geom_point() +
    geom_point(data = BMI_paf_cases_u50_invisible, alpha = 0) +
    facet_manual(~ facet_var, facet_design, respect = T, scales = "free_y") +
    ...
    labs(
      x = "Year",
      y = "Age Specific Incidence Rates per 100,000 (log scale)",
      title = "",
...
ggsave(paste0("../Output/Final_Plots/Figure_4.png"), plot = p, width = 12, height = 14)
ggsave(paste0("../Output/Final_Plots_SVG/Figure_4.svg"), plot = p, width = 12, height = 14)
...
supp_table_4 <- full_joinpoint_table(data_joinpoint |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovarian")) |>
                                       mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site)),
                                  data_joinpoint_AAPC |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovarian")) |>
                                       mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site)),
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  CairoPNG(file=paste0("../Output/Final_Plots/Supplementary_Figure_1", letters[which(unique(data_joinpoint_AAPC$sex == s))], ".png"), width = 10, height = (4/15)*(3 + rows), units = "in", res = 300)
  print(plot)
  ...
  svglite(file=paste0("../Output/Final_Plots_SVG/Supplementary_Figure_1", letters[which(unique(data_joinpoint_AAPC$sex == s))], ".svg"), width = 10, height = (4/15)*(3 + rows))
  print(plot)
...
BMI_paf_cases_oe50_invisible <- BMI_paf_cases_oe50 |>
  ...
  select(year, rate_type, cancer_site, sex, max_value, min_value) |>
  pivot_longer(cols = c(min_value, max_value)) |>
  # Manually gicving minimum values to Ora as it has no attributable cases
  mutate(
    
    value = case_when(
      cancer_site == "Oral" & name == "min_value" ~ 4,
      TRUE ~ value
...
BMI_paf_cases_oe50 <- BMI_paf_cases_oe50 |>
  ...
  # Manually filtering out Oral Non-Attributable cases as they overlap with the Overall and we want to only see overall
  filter(!(cancer_site %in% c("Oral") & rate_type == "Non-Attributable"))
...
p <- ggplot(BMI_paf_cases_oe50, aes(x = year, y = value, colour = rate_type)) +
    ...
    scale_y_log10(breaks = scales::breaks_log(n = 8),
                  labels = scales::label_number(accuracy = 0.1)) +
    scale_x_continuous(
      breaks = seq(2005, 2019, by = 2),  # Define the x-axis breaks
      limits = c(2005, 2019)
    )+
    scale_colour_manual(
      values = c(
...
# Save the plot as a PNG file
ggsave(paste0("../Output/Final_Plots/Supplementary_Figure_5.png"), plot = p, width = 12, height = 12)
ggsave(paste0("../Output/Final_Plots_SVG/Supplementary_Figure_5.svg"), plot = p, width = 12, height = 12)
