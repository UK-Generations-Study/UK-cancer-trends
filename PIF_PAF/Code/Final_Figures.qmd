...
# Load in function for comparing AAPCs
source("Functions/Sub_Analysis/aapc_comparison_functions.R")
...
# Load in function for Lancet theme for ggplot2 objects
source("Functions/Plotting_Functions/plot_lancet.R")
...
# Calculate significant differences between younger and older
data_joinpoint_AAPC_diff <- data_joinpoint_AAPC |>
...
```{r, fig.height=8.5, fig.width=10.5, warning=F}

# Loop through sexes
for(s in unique(data_joinpoint_model$sex)){
  
  # Filter to correct cancer sites in joinpoint model data
  data_joinpoint_model_fig1 <- data_joinpoint_model |>
    filter(sex == s) |>
    filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovary")) |>
    mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site))
  ...
  data_joinpoint_model_fig1 <- data_joinpoint_model_fig1 |>
  ...
  data_joinpoint_plot <- data_joinpoint_model_fig1 |>
    group_by(age_group, sex, cancer_site) |>
    arrange(Segment) |>
    mutate(
  ...
  # Create invisible points so rows have the same y bounds
  invisible_points <- data_cancer |>
    mutate(n_sites = length(unique(cancer_site))) |>
    group_by(cancer_site, sex) |>
    summarise(max = max(Rate), min = min(Rate), max_20_49 = max(Rate*(age_group == "20-49"))) |>
    ungroup() |>
    arrange(desc(max_20_49)) |>
    mutate(facet_row = ceiling(row_number()/4)) |>
    group_by(facet_row) |>
    mutate(y = list(range(c(max, min)))) |>
    unnest(y) |>
    mutate(year = 2019)
  ...
  plot <- ggplot(data_cancer_facet, aes(x = year, y = Rate, colour = age_group)) +
    ...
    geom_point(data = invisible_points, aes(x = year, y = y), alpha = 0, inherit.aes =F) +
    theme_minimal_lancet() +
    scale_colour_manual(values = c("20-49" = "#ff9500", "50+" = "#38b000"),
                        labels = c("20-49 years", "50+ years")) +
    ...
    scale_x_continuous(
      breaks = seq(2001, 2019, by = 3),  # Define the x-axis breaks
      limits = c(2001, 2019)
    )+
    # ggtitle("Cancer Incidence Rates for Selected Cancers") +
    ylab("Age Standardised Incidence Rates per 100,000 (log scale)") +
    xlab("Year") +
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Figure_1", letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".png"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 10.5)
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_1",  letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".svg"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 10.5)
...
```{r, fig.height=6, fig.width=4, warning=F}
...
rf_trend_plot <- data_rf |> 
  ...
  mutate(age_group = if_else(age_group == "20-49", "20-49 years", "50+ years"))
...
jp_plots[["alcohol"]] <- plot_joinpoint(
  data = rf_trend_plot |> filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker") |> filter(year != 2005),
  data.model = rf_model_estimates |> filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker"),
  facet.x = "age_group", facet.y = "sex", colour = "level") +
  geom_point(data = rf_trend_plot |> filter(variable == "alcohol_amt") |> filter(level != "Non-Drinker") |> filter(year == 2005),
             aes(x = year, y = value, colour = level)) +
  labs(
    y = "Prevalence (log scale)",
    colour = "Risk Factor",
  ...
  scale_color_manual( #fixing the colors
    ...
    labels = c(
      "Heavy Drinker" = "Heavy (>50 g/day)",
      "Moderate Drinker" = "Moderate (12-50 g/day)",
      "Light Drinker" = "Light (<12 g/day)"
    ),
    breaks = c(
      "Heavy Drinker",
      "Moderate Drinker",
...
jp_plots[["bmi"]] <- plot_joinpoint(
  ...
  scale_color_manual( #fixing the colors
    ...
    labels = c(
      "Obese" = expression("Obese (">30~"kg/m"^2~")"),
      "Overweight" = expression("Overweight ("~25-30~"kg/m"^2~")")
    )
  )
...
jp_plots[["physical_activity"]] <- plot_joinpoint(
  ...
  labs(
    ...
    caption = md("*150 min/week of moderate or 75 min/week of vigorous activity\n**150 min/week of activity")
  ) +
  theme_minimal_lancet() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  scale_color_manual( #fixing the colors
    ...
    labels = c("physical_activity_old" = "Physical Activity Below old UK Guidelines*",
               "physical_activity" = "Physical Activity Below new UK Guidelines**"),
    breaks = c("physical_activity_old","physical_activity")
  )
...
jp_plots[["meat_consumption"]] <- plot_joinpoint(
  ...
  facet.x = "age_group", facet.y = "sex", colour = "variable") +
  labs(
    y = "Daily Consumption (g) (log scale)",
    colour = "Risk Factor",
...
for(p in names(jp_plots)){
  ...
  # Make legend vertically stack
  jp_plots[[p]] <- jp_plots[[p]] +
  ...
  ggsave(jp_plots[[p]],
       filename = paste0("../Output/Final_Plots/Figure_2", letters[which(names(jp_plots) == p)], ".png"),
       bg = "white",
       height = 6,
       width = 4)
  
   ggsave(jp_plots[[p]],
       filename = paste0("../Output/Final_Plots_SVG/Figure_2", letters[which(names(jp_plots) == p)], ".svg"),
       bg = "white",
       height = 6,
       width = 4)
...
# plot <- (jp_plots$smoking + jp_plots$alcohol)/(jp_plots$bmi + jp_plots$physical_activity)/(jp_plots$fibre_consumption + jp_plots$meat_consumption)
# 
# ggsave(plot,
#        filename = paste0("../Output/Final_Plots/Figure_2.png"),
#        bg = "white",
#        height = 14,
#        width = 8)
...
data_paf <- data_paf_individual %>%
  ...
  summarise(
    ...
    Ovary = calculate_cumulative_paf(OvaryPAF)
...
data_attr_cases_bar_alt <- merge(data_attr_cases_bar_alt, data_paf_agg_ci, by.x = c("cancer_site", "age_group", "sex"), by.y = c("Cancer_sites", "age_group", "sex"), all.x = T) |>
...
data_attr_bar_paf <- ggplot(data_attr_cases_bar_alt, aes(x = cancer_site, y = PAF, fill = age_group)) +
  ...
  labs(
    # title = "Population Attributable Fractions 2019: Aggregate",
    fill = "Age Group",
    x = "Cancer Site",
    y = "PAF (%)"
  )+
...
ggsave("../Output/Final_Plots/Figure_3a.png", plot = data_attr_bar_paf, bg = "white", height = 4, width = 6)
ggsave("../Output/Final_Plots_SVG/Figure_3a.svg", plot = data_attr_bar_paf, bg = "white", height = 4, width = 6)

```

### Figure 4 - BMI Attributable Trends
```{r, fig.height=8.5, fig.width=8, warning=F}

for(s in unique(data_bmi_esp$sex)){
  ...
  BMI_paf_cases <- data_BMI_paf %>%
    ...
    filter(min(attributable_rate) > 0) |>
    ungroup()
  ...
  # # Calculating invisible points so y-axis identical for same cancer site
  # BMI_paf_cases_u50_invisible <- BMI_paf_cases_u50 |>
  #   group_by(cancer_site) |>
  #   mutate(min_value = min(value),
  #          max_value = max(value)) |>
  #   ungroup() |>
  #   filter(year == 2019, rate_type == "Rate") |>
  #   select(year, rate_type, cancer_site, sex, max_value, min_value) |>
  #   pivot_longer(cols = c(min_value, max_value)) |>
  #   # Manually gicving minimum values to Oral and Breast as they have no attributable cases
  #   mutate(
  #     
  #     value = case_when(
  #       cancer_site == "Oral" & name == "min_value" ~ 0.3,
  #       cancer_site == "Breast" & name == "min_value" ~ 8,
  #       TRUE ~ value
  #       
  #     ),
  #     
  #     # Need to switch to other naming system
  #     rate_type = "Overall",
  #     
  #   )
  ...
  BMI_paf_cases_u50_invisible <- BMI_paf_cases_u50 |>
    ...
    ungroup() |>
    arrange(desc(max)) |>
    mutate(facet_row = ceiling(row_number()/3)) |>
    group_by(facet_row) |>
    mutate(y = list(range(c(max, min)))) |>
    unnest(y) |>
    mutate(year = 2019)
  
  facet_vars <- BMI_paf_cases_u50 |>
    group_by(cancer_site) |>
    filter(rate_type == "Rate") |>
    mutate(max = max(value)) |>
    ungroup() |>
    distinct(cancer_site, sex, max) |>
    arrange(desc(max), (sex)) |>
    mutate(facet_var = paste0(cancer_site, " - ", sex),
              n = n()) |>
    select(facet_var) |>
    unique() |>
  ...
  # Add this facet to all dataframes
  BMI_paf_cases_u50_invisible <- BMI_paf_cases_u50_invisible |>
    mutate(
  ...
  BMI_paf_cases_u50 <- BMI_paf_cases_u50 |>
    ...
    # Manually filtering out Oral and Breast Non-Attributable cases as they overlap with the Overall and we want to only see overall
    filter(!(cancer_site %in% c("Oral", "Breast") & rate_type == "Non-Attributable"))
  
  p <- ggplot(BMI_paf_cases_u50, aes(x = year, y = value, colour = rate_type)) +
      geom_line() +
      geom_point() +
      geom_point(data = BMI_paf_cases_u50_invisible, aes(x = year, y = y), inherit.aes = F, alpha = 0) +
      # facet_manual(~ facet_var, facet_design, respect = T, scales = "free_y") +
      ...
      scale_colour_manual(
        values = c(
          "Attributable" = "#ef6f69",
          "Non-Attributable" = "#54b55a",
          "Overall" = "#495057"
        )
      ) +
      labs(
        x = "Year",
        y = "Age Standardised Incidence Rates per 100,000 (log scale)",
        title = "",
        colour = "Incidence Rates"
      ) +
      theme_minimal_lancet() +
      geom_rect(
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Figure_4", letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".png"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_4",  letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".svg"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
...
supp_table_5 <- full_joinpoint_table(data_joinpoint |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovary")) |>
                                       mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site)),
                                  data_joinpoint_AAPC |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovary")) |>
                                       mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site)),
                                  group_var = "cancer_site", table_var = "sex", stratification_vars = "age_group", test_difference = T)
...
for(sex in c("Women", "Men")){
  ...
  supp_table_5[[sex]] <- supp_table_5[[sex]] |>
    ...
    tab_header(paste0("Joinpoint Cancer Incidence Trends in ", sex))

  # Print
  print(supp_table_5[[sex]])
  
  # Save
  gtsave(supp_table_5[[sex]], paste0("../Output/Final_Plots_HTML/Supplementary_Table_5", letters[which(c("Women", "Men") == sex)], ".html"))
  gtsave(supp_table_5[[sex]],  paste0("../Output/Final_Plots/Supplementary_Table_5", letters[which(c("Women", "Men") == sex)], ".docx"))
...
# Create table
supp_table_6 <- full_joinpoint_table(rfdata_joinpoint_complete, rfdata_joinpoint_complete_AAPC, group_var = "group_var", table_var = "sex", stratification_vars = "age_group")
...
for(sex in c("Women", "Men")){
  ...
  # Final changes
  supp_table_6[[sex]] <- supp_table_6[[sex]] |>
    cols_label(age_group = "Age Group") |>
    tab_header(paste0("Joinpoint Trends in Risk Factors ", sex)) |>
  ...
  tab_footnote(
    footnote = "Heavy (>50g/day), moderate (12-50 g/day), light (<12 g/day)",
    locations = cells_row_groups(
      groups = c("Alcohol Consumption - Light Drinker", "Alcohol Consumption - Moderate Drinker", "Alcohol Consumption - Heavy Drinker")
    )
  ) |>
  tab_footnote(
    footnote = "Overweight (>30 kg/m²), obese (25-30 kg/m²)",
    locations = cells_row_groups(
      groups = c("Body Mass Index (BMI) - Obese", "Body Mass Index (BMI) - Overweight")
    )
  ) |>
  tab_footnote(
    footnote = "Recommendations are 150 min/week of moderate or 75 min/week or vigorous activity",
    locations = cells_row_groups(
      groups = c("Physical Inactivity - Below UK Recommendations")
    )
  ) |>
  tab_footnote(
    footnote = "Guidelines are 30g of fibre per day",
    locations = cells_row_groups(
      groups = c("Fibre Intake Deficiency - Below UK Recommendations")
    )
  )
  
  # Print
  print(supp_table_6[[sex]])
  
  # Save
  gtsave(supp_table_6[[sex]], paste0("../Output/Final_Plots_HTML/Supplementary_Table_6", letters[which(c("Women", "Men") == sex)], ".html"))
  gtsave(supp_table_6[[sex]], paste0("../Output/Final_Plots/Supplementary_Table_6", letters[which(c("Women", "Men") == sex)], ".docx"))

}

```

### Supplementary Table 7 - AAPC and APC Trends in BMI Attributable Rates
```{r, results = "asis"}

bmi_paf_apc <- read.delim("../Data/Joinpoint_Results/BMI_Attributable_new/jp_bmi_attributable.Export.APC.txt") |>
...
bmi_paf_aapc <- read.delim("../Data/Joinpoint_Results/BMI_Attributable_new/jp_bmi_attributable.Export.AAPC.txt") |>
  mutate("sex-age_group" = paste0(sex, ", ", age_group)) |>
  mutate(rate_type = case_when(
    rate_type == "rate" ~ "Overall",
    rate_type == "attributable_rate" ~ "Attributable",
...
plot_order <- c("Women, 20-49", "Men, 20-49", "Women, 50+", "Men, 50+")

for(group in plot_order){
  
  bmi_jp_table[[group]] <- bmi_jp_table[[group]] |>
    cols_width(rate_type ~ px(145)) |>
    cols_label("rate_type" ~ "Rate") |>
    tab_header(paste0("Joinpoint Trends in BMI-Attributable Cases in ", group))
  
  print(bmi_jp_table[[group]])
  
  gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots_HTML/Supplementary_Table_7", letters[which(plot_order == group)], ".html"))
  gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots/Supplementary_Table_7", letters[which(plot_order == group)], ".docx"))
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  data_joinpoint_AAPC_sf2 <- data_joinpoint_AAPC |>
      merge(data_joinpoint_AAPC_diff|> filter(sex == s) |> select(cancer_site, p_value_one_sided), by = "cancer_site") |>
    filter(sex == s) |>
    group_by(cancer_site) |>
    ...
    mutate(
      cancer_site = if_else(p_value_one_sided < 0.05 & AAPC.C.I..Low[2] > 0, paste0(cancer_site[2], "^"), cancer_site[2])
    )
  ...
  plot <- data_joinpoint_AAPC_sf2 |>
    mutate(
      ...
      cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site),
      P.Value = if_else(P.Value < 0.001, "<0.001", as.character(sub("\\.?0+$", "", format(signif(P.Value, digits = 2), scientific = FALSE)))),
      cancer_site = factor(cancer_site, levels = data_jp_aapc_order$cancer_site),
      age_group = factor(age_group, levels = c("50+", "20-49"), labels = c("50+ years", "20-49 years")),
    ...
    forestplot(
      ...
      upper = AAPC.C.I..High,
      xlab = "AAPC",
      title = paste0("Cancer Incidence AAPC Values - ", s),
      line.margin = 0.2,
      legend = c("20-49 years", "50+ years"),
      txt_gp = fpTxtGp(
        title = gpar(fontsize = 10, fontfamily = "Times New Roman", fontface = "bold"),
        xlab = gpar(fontsize = 16, fontfamily = "Times New Roman", fontface = "bold"),
        label = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"),
        legend = gpar(fontsize = 14, fontfamily = "Times New Roman", fontface = "plain"),
        ticks = gpar(fontsize = 16, fontfamily = "Times New Roman", fontface = "plain")
  ...
  # Add footnote
  grid.text("* - AAPC for 20-49 is significantly greater than 0", x = 0.095, y = 0.02, gp = gpar(fontsize = 8))
  grid.text("^ - AAPC for 20-49 is significantly greater than both 0 and the AAPC for 50+", x = 0.195, y = 0.008, gp = gpar(fontsize = 8))
  
  # Save plot
  
  CairoPNG(file=paste0("../Output/Final_Plots/Supplementary_Figure_1", letters[which(unique(data_joinpoint_AAPC$sex == s)) %% 2 + 1], ".png"), width = 10, height = (4/15)*(3 + rows), units = "in", res = 300)
  print(plot)
  grid.text("* - AAPC for 20-49 is significantly greater than 0", x = 0.135, y = 0.03, gp = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"))
  grid.text("^ - AAPC for 20-49 is significantly greater than both 0 and the AAPC for 50+", x = 0.208, y = 0.01, gp = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"))
  dev.off()
  
  svglite(file=paste0("../Output/Final_Plots_SVG/Supplementary_Figure_1", letters[which(unique(data_joinpoint_AAPC$sex == s)) %% 2 + 1], ".svg"), width = 10, height = (4/15)*(3 + rows))
  print(plot)
  grid.text("* - AAPC for 20-49 is significantly greater than 0", x = 0.13, y = 0.03, gp = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"))
  grid.text("^ - AAPC for 20-49 is significantly greater than both 0 and the AAPC for 50+", x = 0.208, y = 0.01, gp = gpar(fontsize = 9, fontfamily = "Times New Roman", fontface = "plain"))
  dev.off()
...
plot_order <- c("Cigarette Smoking - Previously", "Cigarette Smoking - Current", "Alcohol Consumption - Light Drinker", "Alcohol Consumption - Moderate Drinker", "Alcohol Consumption - Heavy Drinker", "BMI - Overweight", "BMI - Obese", "Physical Inactivity - Below Old Recommendations")
...
for(var in plot_order){
  ...
  plot <- ggplot(rf_imd_trend_plot |> mutate(complete = paste0(variable, " - ", level)) |> filter(complete == var), aes(x = year, y = value, color = as.factor(imd), group = imd)) +
    geom_line(size = 1) +
    labs(
      title = md(paste0("Risk Factor Prevalence by IMD:\n", variable_clean)),
      x = "Year",
      y = "Value (log scale)",
      color = "IMD Status"
    ) +
  ...
  # Created unannotated plot
  g <- ggplotGrob(plot)
  
  # Loop through grobs, removing labels if seen
  for(i in 1:length(g$grobs)){
    
    if("gTree" %in% class(g$grobs[[i]])){
      
      label_ids <- grep("geom_label", names(g$grobs[[i]]$children))
      
      if(length(label_ids) > 0){
        
        g$grobs[[i]]$children <- g$grobs[[i]]$children[-label_ids]
  ...
  svglite(paste0("../Output/Final_Plots_SVG/Supplementary_Figure_3", letters[which(plot_order == var)], "_unannotated.svg"), width = 6, height = 4, bg = "white")
  grid.draw(g)
  dev.off()
  
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_3", letters[which(plot_order == var)], ".png"), plot = plot, width = 6, height = 4, bg = "white")
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_3", letters[which(plot_order == var)], ".svg"), plot = plot, width = 6, height = 4, bg = "white")
...
data_attr_cases_bar <- data_attr_cases_bar |>
  ...
  mutate(
    ...
    variable = ifelse(variable == "BMI", "Body Mass Index (BMI)", variable),
    variable = ifelse(variable == "Physical Activty", "Physical Inactivity", variable),
...
plot_order <- c("Cigarette Smoking", "Alcohol Consumption", "Body Mass Index (BMI)", "Physical Inactivity", "Fibre Intake Deficiency", "Red Meat Consumption", "Processed Meat Consumption")
...
for (var in plot_order) {
  ...
  data_attr_bar_paf <- ggplot(subset_data, aes(x = cancer_site, y = PAF, fill = age_group)) +
    ...
    labs(
      title = md(paste0("Population Attributable Fractions 2019:\n", var)),
      fill = "Age Group",
      x = "Cancer Site",
      y = "PAF (%)"
    )+
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_4", letters[which(plot_order == var)], ".png"), plot = data_attr_bar_paf, width = 6, height = 4, bg = "white")
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_4", letters[which(plot_order == var)], ".svg"), plot = data_attr_bar_paf, width = 6, height = 4, bg = "white")
  
  if(var == "Body Mass Index (BMI)"){
    ...
    ggsave(filename = paste0("../Output/Final_Plots/Figure_3b.png"), plot = data_attr_bar_paf, width = 6, height = 4, bg = "white")
    ggsave(filename = paste0("../Output/Final_Plots_SVG/Figure_3b.svg"), plot = data_attr_bar_paf, width = 6, height = 4, bg = "white")
  }
...
for(s in unique(data_bmi_esp$sex)){
  ...
  BMI_paf_cases_oe50 <- BMI_paf_cases |>
    filter(sex == s) |>
    filter(age_group == "50+") |>
    filter(Rate > 0) |>
    pivot_longer(cols = c("attributable_rate", "Rate", "non_attributable_rate"), names_to = "rate_type") |>
  ...
  # # Calculating invisible points so y-axis identical for same cancer site
  # BMI_paf_cases_oe50_invisible <- BMI_paf_cases_oe50 |>
  #   group_by(cancer_site) |>
  #   mutate(min_value = min(value),
  #          max_value = max(value)) |>
  #   ungroup() |>
  ...
  # Add this facet to all dataframes
  BMI_paf_cases_oe50 <- BMI_paf_cases_oe50 |>
    mutate(
  ...
  # Add this facet to all dataframes
  BMI_paf_cases_oe50_invisible <- BMI_paf_cases_oe50_invisible |>
    mutate(
  ...
  p <- ggplot(BMI_paf_cases_oe50, aes(x = year, y = value, colour = rate_type)) +
      geom_line() +
      geom_point() +
      geom_point(data = BMI_paf_cases_oe50_invisible, aes(x = year, y = y), inherit.aes = F, alpha = 0) +
      # facet_manual(~ facet_var, facet_design, respect = T, scales = "free_y") +
      facet_wrap(~ facet_var, scales = "free_y", ncol = 3) +
      scale_y_log10(breaks = scales::breaks_log(n = 8),
                    labels = scales::label_number(accuracy = 0.01)) +
  ...
  # Save the plot as a PNG file
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_5", letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".png"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_5",  letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".svg"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 8)
...
ggsave("../Output/Final_Plots/Supplementary_Figure_6.png", plot = pafplotall, bg = "white", height = 4, width = 6)
ggsave("../Output/Final_Plots_SVG/Supplementary_Figure_6.svg", plot = pafplotall, bg = "white", height = 4, width = 6)
...
data_cancer <- data_ALLcancer %>%
  ...
  filter(year == 2019) |>
  group_by(cancer_site, sex) |>
  mutate(Rate = 100*Rate/sum(Rate)) |>
  ungroup() |>
  mutate(age_group = factor(age_group, levels = c("50+", "20-49"))) |>
  mutate(sex = factor(sex, levels = c("Women", "Men"))) |>
  mutate(Rate_order = if_else(age_group == "20-49" & sex == "Women", 0, Rate)) |>
  arrange(desc(Rate_order)) |>
  mutate(cancer_site = factor(cancer_site, levels = unique(cancer_site)))


plot <- ggplot(data_cancer, aes(x = cancer_site, y = Rate, fill = age_group)) +
  geom_bar(position = "stack", stat = "identity", colour = "black") +
  coord_flip() +
  facet_wrap(~ sex) +
  scale_fill_manual(breaks = c("20-49", "50+"),
                    values = c("20-49" = "#ff9500", "50+" = "#38b000"),
                      labels = c("20-49" ="20-49 years", "50+" ="50+ years")) +
    theme_minimal_lancet() +
    theme(
    ...
    labs(
      # title = "Proportion of Cancer Burden by Age Group, 2019",
      fill = "Age Group",
      x = "Cancer Site",
      y = "Proportion of Cancer Incidence by Age Group (%)"
    )+
    geom_rect(xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = Inf,
              colour = "black", fill = NA, inherit.aes = FALSE) +
    theme(strip.text = element_text(face = "bold"),
...
ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_7.png"), plot = plot, height = 4, width = 6, bg = "white")
ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_7.svg"), plot = plot, height = 4, width = 6, bg = "white")
...
```{r, fig.height= 14, fig.width = 8, warning = F}
...
for(s in c("Women", "Men")){
  ...
  for(var in c("BMI (20-49 years)", "BMI (2-15 years)", "Alcohol Consumption", "Physical Inactivity", "Fibre Intake Deficiency", "Meat Consumption")){
    ...
    if(var %in% c("BMI (20-49 years)", "BMI (2-15 years)")){
      ...
      plot_rf <- plot_rf +
        scale_colour_manual(
          ...
          labels = c(
            "Obese" = expression("Obese"^"a"),
            "Overweight" = expression("Overweight"^"a")
          )
        )
      ...
      plot_rf <- plot_rf +
        scale_color_manual( #fixing the colors
          ...
          labels = c(
            "Heavy Drinker" = expression("Heavy Drinker"^"c"),
            "Moderate Drinker" =  expression("Moderate Drinker"^"c"),
            "Light Drinker" =  expression("Light Drinker"^"c")
          ),
          breaks = c(
            "Heavy Drinker",
            "Moderate Drinker",
      ...
      plot_rf <- plot_rf +
        scale_color_manual( #fixing the colors
          ...
          labels = c(
            "Below Recommendations" = expression("Below Recommendations"^"d")
          )
          # labels = c(
          #   # "physical_activity" = "Physical Activity Below new UK Guidelines**",
      ...
      plot_rf <- plot_rf +
        scale_color_manual(
          values = c("Below Guidelines" = "#4f518c"),
          labels  = c("Below Guidelines" = expression("Below Guidelines"^"e"))
          # labels = c("Below Guidelines" = "Fibre Consumption Below UK Guidelines (30g/day)")
        )
    ...
    if(var == "BMI (2-15 years)"){
      plot_rf <- plot_rf +
        ...
        scale_colour_manual(
          ...
          labels = c(
            "Obese" = expression("Obese"^"b"),
            "Overweight" = expression("Overweight"^"b")
          )
        )
    } else {
      plot_rf <- plot_rf +
    ...
    if(var == "Meat Consumption"){
      plot_rf <- plot_rf +
        labs(x = "Year", y = md("Median Daily\nConsumption (g)"),
             caption = "             ᵃ Overweight (>30 kg/m²), obese (25-30 kg/m²)\n
             ᵇ Overweight , obese\n
             ᶜ Heavy (>50g/day), moderate (12-50 g/day), light (<12 g/day)\n
             ᵈ Recommendations are 150 min/week of moderate or 75 min/week or vigorous activity\n
             ᵉ Guidelines are 30g of fibre per day"
             ) +
  ...
  #   draw_label(paste0("Trends in Colorectal Cancer and Risk Factors in Younger ", s, " aged 20-49"), fontface = 'bold', fontfamily = "Times New Roman", 
  #              x = 0, y = 0.99,  
  #              hjust = -0.05, vjust = 1,  
  #              size = 10)
  ...
  ggsave(filename = paste0("../Output/Final_Plots/Supplementary_Figure_8", letters[which(c("Women", "Men") == s)], ".png"), plot = plot, width = 9, height = 14, bg = "white")
  ggsave(filename = paste0("../Output/Final_Plots_SVG/Supplementary_Figure_8", letters[which(c("Women", "Men") == s)], ".svg"), plot = plot, width = 9, height = 14, bg = "white")
...
### Figure 1 - Joinpoint trends of selected cancer sites (Linear y-axis)
...
for(s in unique(data_joinpoint_model$sex)){
  ...
  data_joinpoint_plot <- data_joinpoint_model_fig1 |>
    ...
    mutate(
      ...
      start.y = exp(Intercept.Estimate + start.year * Slope.Estimate),
      end.y = ifelse(Segment == max(Segment), 
                     exp(Intercept.Estimate + end.year * Slope.Estimate), 
                     lead(start.y))
    ) |>
    ungroup() |>
    select(all_of(c("age_group", "sex", "cancer_site", "start.year", "end.year", 
                    "start.y", "end.y", "Intercept.Estimate", "Slope.Estimate"))) |>
    rowwise() |>
    mutate(
      n_points = (end.year - start.year) * 100,
      year = list(seq(start.year, end.year, length.out = n_points)),
      y    = list(exp(Intercept.Estimate + seq(start.year, end.year, length.out = n_points) * Slope.Estimate))
    ) |>
    ungroup() |>
    select(-c(start.year, end.year, start.y, end.y, Intercept.Estimate, Slope.Estimate, n_points)) |>
    unnest(c(year, y))
  ...
  ggsave(filename = paste0("../Output/Additional_Plots/Figure_1", letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".png"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 10.5)
  ggsave(filename = paste0("../Output/Additional_Plots_SVG/Figure_1",  letters[which(unique(data_joinpoint_model$sex == s)) %% 2 + 1], ".svg"), bg = "white", height = if_else(s == "Women", 8.5, 6.5), width = 10.5)
...
### Table of relative changes in BMI PAF in young people
```{r}

# Calculate the PAFs by cancer site 
pafs_byrf_under50<- paf_calculation(data_rf, "20-49")

additional_table <- pafs_byrf_under50 |>
  pivot_longer(cols = all_of(ends_with("PAF")), names_to = "cancer_site", values_to = "PAF") |>
  mutate(cancer_site = gsub("PAF*", "", cancer_site)) |>
  group_by(sex, cancer_site) |>
  filter(year %in% c(2001, 2019)) |>
  filter(variable == "BMI") |>
  arrange(desc(year)) |>
  summarise(PAF_relative_change = first(PAF)/last(PAF)) |>
  mutate(PAF_relative_change = 100*(PAF_relative_change - 1)) |>
  filter(!is.na(PAF_relative_change))

additional_table

write.csv(additional_table, "../Output/Additional_Plots/Table.csv", row.names = T)

```


### Table of changes in BMI from 2001 to 2019
```{r}

data_rf_temp <- read.csv("../../../Data/Cleaned_Data/clean_rf_data.csv")

table <- data_rf_temp |>
  filter(year %in% c(2001, 2019) & age_group == "20-49" & variable == "bmi") |>
  select(-N) |>
...
write.csv(additional_table, "../Output/Additional_Plots/Table_2.csv", row.names = T)

```

### Table of BMI PAF values 2001 and 2019
```{r}
...
write.csv(additional_table, "../Output/Additional_Plots/Table_3.csv", row.names = T)

```

### Number of bowel cancer cases in men and women under 50 from 2001-2019
```{r}

additional_table <- read.csv("../Data/Incidence_data_National_ICD10_2026-04-29.csv") |>
  mutate(age_group = "20-49") |>
  mutate(cancer_site = "Colorectum") |>
  select(year = Year, gender = Gender, age_group, cancer_site, count = Count) |>
  group_by(year, gender, age_group, cancer_site) |>
  summarise(count = sum(count))

write.csv(additional_table, "../Output/Additional_Plots/Table_4.csv")
