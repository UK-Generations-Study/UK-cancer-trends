...
supp_table_4 <- full_joinpoint_table(data_joinpoint |> filter(cancer_site %in% c("Oral", "Endometrium", "Pancreas", "Gallbladder", "Colorectum", "Liver", "Kidney", "MultipleMyeloma", "Breast", "Thyroid", "Ovary")) |>
                                       mutate(cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site)),
...
for(sex in c("Women", "Men")){
  ...
  # Save
  gtsave(supp_table_4[[sex]], paste0("../Output/Final_Plots_HTML/Supplementary_Table_4", letters[which(c("Women", "Men") == sex)], ".html"))
  gtsave(supp_table_4[[sex]], paste0("../Output/Final_Plots/Supplementary_Table_4", letters[which(c("Women", "Men") == sex)], ".docx"))
  # gtsave(supp_table_4[[sex]], paste0("../Output/Final_Plots_PDF/Supplementary_Table_4", letters[which(c("Women", "Men") == sex)], ".pdf"))
...
for(sex in c("Women", "Men")){
  ...
  # gtsave(supp_table_5[[sex]], paste0("../Output/Final_Plots_PDF/Supplementary_Table_5", letters[which(c("Women", "Men") == sex)], ".pdf"))
  
}

```

### Supplementary Table 6 - AAPC and APC Trends in BMI Attributable Rates
```{r, results = "asis"}
...
for(group in plot_order){
  ...
  gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots_HTML/Supplementary_Table_6", letters[which(plot_order == group)], ".html"))
  gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots/Supplementary_Table_6", letters[which(plot_order == group)], ".docx"))
  # gtsave(bmi_jp_table[[group]], paste0("../Output/Final_Plots_PDF/Supplementary_Table_6", letters[which(plot_order == group)], ".pdf"))
  
}

```

### Supplementary Table 7 - PubMed search terms
...
for(s in unique(data_joinpoint_AAPC$sex)){
  ...
  plot <- data_joinpoint_AAPC_sf2 |>
    mutate(
      ...
      cancer_site = if_else(cancer_site == "MultipleMyeloma", "Multiple Myeloma", cancer_site),
      P.Value = if_else(P.Value < 0.0001, "<0.0001", as.character(sub("\\.?0+$", "", format(signif(P.Value, digits = 2), scientific = FALSE)))),
