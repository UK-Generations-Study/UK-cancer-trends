...
for (site in unique(BMI_paf_cases$cancer_site)) {
  ...
  df_filtered <- BMI_paf_cases %>% 
    ...
    mutate(PAF = PAF * 100)  # Convert PAF to percentage
  
  # Create the plot
  p <- ggplot(df_filtered, aes(x = factor(year))) +  
    # Bar chart for PAF
    geom_col(aes(y = PAF), fill = "skyblue", alpha = 0.7) + 
    
    # Line and points for Attributable cases 
    geom_line(aes(y = attributable / max(attributable, na.rm = TRUE) * max(PAF, na.rm = TRUE)), 
              group = 1, color = "red", size = 1) + 
    geom_point(aes(y = attributable / max(attributable, na.rm = TRUE) * max(PAF, na.rm = TRUE)), 
               color = "red", size = 3) +
    
    # Y-axis
    scale_y_continuous(
      name = "PAF (%)",
      sec.axis = sec_axis(~ . * (max(df_filtered$attributable, na.rm = TRUE) / 
                                 max(df_filtered$PAF, na.rm = TRUE)), 
