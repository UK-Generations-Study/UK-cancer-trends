# This is the PIF calculation function (BMI only)
...
pif_calculation_bmi <- function(data_paf) {
  ...
  data_paf <- data_paf |>
    filter(variable == "BMI") |>
    filter(year %in% c(1995, 2005, 2009, 2019)) |>
    ...
    summarise(
      ...
      PIF_past = if_else(PIF_component[1] == 1 | PIF_component[3] == 1, NA, (PIF_component[3] - PIF_component[1])/PIF_component[3]),
      
      PIF_future = if_else(PIF_component[2] == 1 | PIF_component[4] == 1, NA, (PIF_component[4] - PIF_component[2])/PIF_component[4]),
