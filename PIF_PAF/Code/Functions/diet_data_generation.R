...
diet_data_gen <- function(filepath){
  ...
  # Initialise empty df
  diet_df_total <- data.frame(sex = character(0), age_group = character(0), year = numeric(0), fibre_cat = character(0), perc = numeric(0))
  ...
  diet_df_fibre <- diet_df |>
    ...
    summarise(perc = sum(weight)/total_weight[1]) |>
