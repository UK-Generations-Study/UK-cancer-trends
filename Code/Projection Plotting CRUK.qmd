...
### JP GT Table
```{r}
create_gt_jp_table <- function(jp_data, cancer, sex = "All People", incmort){
  
  # browser()
  
  jp_data <- jp_data |>
    filter(Sex == sex, Cancer.Type == cancer) |>
    arrange(Year)
  
  # This could potentially throw errors if an APC is repeated. This is very unlikely but there is an error output in case.
  # In future should make the function proof of this error, even if it is unlikely. 
  APCs <- unique(jp_data$APC)
  APCs <- APCs[!is.na(APCs)]
  
  years_jp <- jp_data |>
    filter(!is.na(Flag)) |>
    pull(Year)
  
  start.year <- min(jp_data$Year)
  
  end.year <- max(jp_data$Year)
  
  if(length(APCs) != length(years_jp)+1){
    stop("ERROR in Data")
  }
  
  start.years <- c(start.year, years_jp)
  end.years <- c(years_jp, end.year)
  
  output_df <- data.frame(`Start Year` = start.years, `End Year` = end.years, APC = APCs) |>
    mutate(Section = paste0("Segment ", row_number())) |>
    relocate(Section)
  ...
  output_gt <- output_df |>
    ...
    # Header Creation
    tab_header(
      title = md(paste0("**JoinPoint Analysis for ", cancer, " Cancer ", incmort, " in " , sex, "**"))
    ) |>
    ...
    tab_style(
      ...
      locations = cells_body(
          columns = colnames(output_df)# [colnames(output_df)!="Section"]
      )
    ) |>
    ...
    tab_style(
      style = list(
        cell_borders(sides = "right", weight = px(2), color = "lightgrey")
      ),
      locations = cells_body(
        column = Section
      )
    ...
    # Add footer
    tab_footnote(
      footnote = "* Indicates an APC with p value < .05",
      locations = cells_column_labels(
        columns = APC
      )
    )
...
for(cancer in cancers){
  if(cancer == "Breast"){
    p1 <- plot_joinpoint_output_CRUK_alt(cancer = cancer, sex = "Women", data = data_inc, data_jp = incidence_jp, incmort = "Incidence", report.APC = TRUE)
    ...
    p1 <- plot_joinpoint_output_CRUK_alt(cancer = cancer, sex = "Men", data = data_inc, data_jp = incidence_jp, incmort = "Incidence", report.APC = TRUE)
    p2 <- plot_joinpoint_output_CRUK_alt(cancer = cancer, sex = "Men", data = data_mort, data_jp = mortality_jp, incmort = "Mortality",
                                     projection.yr = 2040, projection.target = 0.85, cruk_proj = proj_mort, start.yr = 1993, moonshot.target = TRUE,
                                     report.APC = TRUE)
  } else {
    p1 <- plot_joinpoint_output_CRUK_alt(cancer = cancer, data = data_inc, data_jp = incidence_jp, incmort = "Incidence", report.APC = TRUE)
...
create_gt_jp_table(incidence_jp, cancer = "Breast", incmort = "Incidence", sex = "Women")
create_gt_jp_table(mortality_jp, cancer = "Breast", incmort = "Mortality", sex = "Women")
