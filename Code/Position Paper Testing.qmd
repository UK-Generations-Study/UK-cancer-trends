...
plot_joinpoint_output <- function(data, projection.yr = NA){
  ...
  plot_inc <- ggplot(data_inc, aes(x = Year, y = ASR)) +
    ...
    scale_x_continuous(breaks = seq(min(data_inc$Year), max(data_inc$Year), by = 3))
  ...
  plot_mort <- ggplot(data_mort, aes(x = Year, y = ASR)) +
    ...
    scale_x_continuous(breaks = seq(min(data_mort$Year), max(data_mort$Year), by = 3))
  ...
  combined_plot <- wrap_plots(
    ...
    heights = c(0.07, length(unique(data$group))/2),  # Control height of labels and plots
    widths = c(1,2),
    ncol = 1
...
plot_joinpoint_output(data = total_jp, projection.yr = 2040) +
  plot_annotation(title = "TEST", theme = theme(plot.title = element_text(hjust = 0.5)))
