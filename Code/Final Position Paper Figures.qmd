...
data_1_4 <- merge(data_1_4, data_weights, by = "seriali") |>
  select(surveyyear = SurveyYear, totalredmeat, aoac_fibre = AOACFibreg, sex = Sex, age = Age, weight, country = Country)
...
data_9_11 <- merge(data_9_11, data_weights, by = "seriali") |>
  select(surveyyear = SurveyYear, totalredmeat, aoac_fibre = AOACFibreg, sex = Sex, age = AgeR, weight, country = Country)
...
data_ndhns |>
  ...
  summarise(mean_weight = mean(weight), fibre_rec = sum(fibre_rec)/sum(weight)) |>
...
data_ndhns |>
  group_by(surveyyear_cat, sex, country) |>
  mutate(redmeat_rec = weight*(totalredmeat < 70)) |>
  summarise(mean_weight = mean(weight), redmeat_rec = sum(redmeat_rec)/sum(weight)) |>
  ggplot(aes(x= surveyyear_cat, y = redmeat_rec, colour = country)) +
  geom_point() +
  geom_line() +
  xlab("Year") +
  ylab("% Below Red Meat Intake Guidelines") +
  ggtitle("% Below Red Meat Intake Guidelines* in UK") +
  labs(colour = "Country", caption = "*Guidelines are 70g of Red Meat per day") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major.x = element_line(color = "grey92", size = 0.5),  # Adjust grid lines
        panel.grid.minor.x = element_blank()) +
  facet_wrap(~sex, scales = "fixed", dir = "v")  +
...
# Save plot
ggsave(filename = paste0(output_filepath, "Extra Supplement Figure 1b - Red Meat.png"), width = 8, height = 9, bg = "white")
