...
plot_joinpoint_output <- function(data, projection.yr = NA){
  ...
  # Grabbing legend to use between plots
  legend <- get_legend(
    ggplot(data_inc, aes(x = Year, y = ASR)) +
    geom_line(aes(colour = Country, linetype = last_join)) +
    geom_point(data = subset(data_inc, proj_data == F), aes(colour = Country, shape = proj_data)) +
    geom_point(data = invisible_points_inc, aes(x = Year, y = ASR), alpha = 0) + 
    guides(linetype = "none", shape = "none") +
  ...
  combined_plot <- wrap_plots(
    wrap_elements(textGrob("Incidence", gp = gpar(fontsize = 11))) + wrap_elements(textGrob("Mortality", gp = gpar(fontsize = 11))) + plot_layout(ncol = 2, widths = c(1,2.294118)),
    plot_inc + plot_mort + plot_layout(ncol = 2, widths = c(1,2.294118)),
    heights = c(0.05, length(unique(data$group))/2),  # Control height of labels and plots
    widths = c(1,2),
    ncol = 1
...
# Read in data
incidence_jp <- read.delim(r"(C:\Users\rfrost\OneDrive - The Institute of Cancer Research\Documents\UK-cancer-trends\Data\Cancer Trends\Source Data\Cancer_Incidence_Data_Rolling_Avg_Joinpoint.txt)", sep = "\t") |>
...
# Clean data
figure3a_data_smoking <- smoking_data |>
  filter(
...
figure3a_data_alcohol <- alcohol_data |>
  filter(
...
# Clean obesity data
figure3a_data_obesity <- obesity_data |>
...
# Need to grab legend from the plots
figure3a_legend <- get_legend(ggplot(figure3a_data_obesity, aes(x = year, y = value, colour = country)) +
  geom_line() +
  geom_point())
...
figure3b_data_activity <- activity_data |>
  filter(
...
# Need to grab legend from the plots
# figure3a_legend <- get_legend(ggplot(figure3b_data_activity, aes(x = year, y = value, colour = country)) +
#   geom_line() +
#   geom_point() +
#   labs(colour =  "Country"))
