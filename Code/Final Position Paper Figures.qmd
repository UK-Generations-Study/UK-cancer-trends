...
# Read in data
incidence_jp <- read.delim(r"(Cancer Trends\Source Data\Cancer_Incidence_Data_Rolling_Avg_Joinpoint.txt)", sep = "\t") |>
  filter(Cancer_Site == "All sites excl. NMSC") |>
  rename(ASR = ASR_rolling) |>
  mutate(incmort = "Incidence")

mortality_jp <- read.delim(r"(Cancer Trends\Source Data\Cancer_Mortality_Data_Rolling_Avg_Joinpoint.txt)", sep = "\t") |>
  filter(Cancer_Site == "All sites excl. NMSC") |>
  rename(ASR = ASR_rolling) |>
