...
rf_colors <- c(
  "formersmoker_men" = "#ffb627", 
  "smoker_men" = "#e2711d", 
  "lightalcohol_men" = "#a2d6f9", 
  "medalcohol_men" = "#1e96fc", 
  "heavyalcohol_men" = "#072ac8", 
  "obese_men" = "#7cb518", 
  "overweight_men" = "#058c42"
)

rf_colors_w <- c(
  "formersmoker_women" = "#ffb627", 
  "smoker_women" = "#e2711d", 
  "lightalcohol_women" = "#a2d6f9", 
  "medalcohol_women" = "#1e96fc", 
  "heavyalcohol_women" = "#072ac8", 
  "obese_women" = "#7cb518", 
  "overweight_women" = "#058c42"
)
...
prev_englandwomen <- ggplot(cleanedprev %>% 
  ...
  labs(title = "Risk Factors for Women",
       x = "Year",
       y = "Risk Factor Prevalence") +
  scale_color_manual(values = rf_colors_w) +
  theme_minimal()
...
fibrebar <- ggplot(fibre_cat, aes(x = fibrecat, y = proportion, fill = sex)) +
  ...
  scale_fill_manual(values = c("Men" = "#a8c256", "Women" = "#c33149"))
print(fibrebar)
...
#All cancer sites for men and women 
allcancerin <- cancerrates %>% 
  filter(Cancer_Site == "All sites excl. NMSC", Country == "England", Year <=2019, Year >=2005)
allcancercases <- ggplot(allcancerin, aes(x = Year, y = Incidences, color = Sex, group = Sex)) +
  geom_point() + 
  geom_line() +
  labs(title = "All Cancer Incidence in England",
       x = "Year Diagnosed",
       y = "Incidence Cases") +
  scale_color_manual(values = cancer_colors) +
  scale_x_continuous(breaks = seq(min(allcancerin$Year), max(allcancerin$Year), by = 5)) +
  theme_minimal()
print(allcancercases)

#MEN
cancerratesEM <- cancerrates %>% 
  filter(Country == "England", Sex =="Men", Year <= 2019,Year >=2005,
         !(Cancer_Site %in% c("All sites excl. NMSC", "Oesophageal", "NA"))
         )
...
cancercases_englandmen <- ggplot(cancerratesEM, aes(x = Year, y = Incidences, color = Cancer_Site)) + 
  geom_point() + 
  geom_line() +
  labs(title = "Cancer Site  Incidences in England for Men",
       x = "Year Diagnosed",
       y = "Incidence Cases") +
  scale_color_manual(values = cancer_colors) +
  scale_x_continuous(breaks = seq(min(cancerratesEM$Year), max(cancerratesEM$Year), by = 5)) +
  theme_minimal()
print(cancercases_englandmen)


#WOMEN
cancerratesEW <- cancerrates %>% 
  filter(Country == "England", Sex =="Women", Year <= 2019, Year >=2005, 
         !(Cancer_Site %in% c("All sites excl. NMSC", "Oesophageal", "NA"))
  )
...
cancercases_englandwomen <- ggplot(cancerratesEW, aes(x = Year, y = Incidences, color = Cancer_Site)) + 
  geom_point() + 
  geom_line() +
  labs(title = "Cancer Site  Incidences in England for Women",
       x = "Year Diagnosed",
       y = "Incidence Cases") +
  scale_color_manual(values = cancer_colors) +
   scale_x_continuous(breaks = seq(min(cancerratesEW$Year), max(cancerratesEW$Year), by = 5)) +
  theme_minimal()
...
redmeat_colorectalmen <- ggplot(redmeat, aes( x = yr, y = Colorectal)) +
  geom_line(color = "#ffbe0b") + 
  geom_point() + 
  labs(title = "Red Meat", x = "Year of Exposure", y = "PAF") + 
  theme_minimal () + ylim(0,0.2) +xlim(2005, 2019)
processedmeat_colorectalmen <- ggplot(processedmeat,aes( x = yr, y = Colorectal)) +
  geom_line(color = "#29bf12") + 
  geom_point() + 
...
dietaryMEN <- arrangeGrob(processedmeat_colorectalmen, fibre_colorectalmen, redmeat_colorectalmen, ncol = 3, top = "MEN")
dietaryWOMEN <- arrangeGrob(processedmeat_colorectalwomen, fibre_colorectalwomen, redmeat_colorectalwomen, ncol = 3, top = "WOMEN")
...
pafmen_table_formatted <- pafmen_table %>%
  kable(col.names = c("Year", colnames(pafmen_table)[-1]),
        format = "pipe",
        digits = 2,
        align = "c")

print(pafmen_table_formatted)
...
pafwomen_table_formatted <- pafwomen_table %>%
  kable(col.names = c("Year", colnames(pafwomen_table)[-1]),
        format = "pipe",
        digits = 2,
        align = "c")

print(pafwomen_table_formatted)
```
...
```{r}
relativechangew <- aggregate_PAF_all %>%
  filter(sex == "Women") %>%
  filter((cancersite == "Colorectal" & Year %in% c(2008, 2018)) |
         (cancersite != "Colorectal" & Year %in% c(2005, 2019))) %>%
  pivot_wider(names_from = Year, values_from = PAF, values_fill = list(PAF = 0)) %>%
...
Here are the actual smoking PAFs in 2019 vs. the PAFS with total smoking cessation (assumptions outlined above) for each cancer site.
...
```{r}
mpafalc<- ggplot(data.frame(prevalence = mlightalc_range, PAF = malcprev$SCCpaf), aes(x = prevalence, y = PAF)) +
  geom_line(color = "#568259", size = 1) +
  labs(title = "Men",
  ...
  xlim(.45,.9) +
  theme_minimal() 
wpafalc<- ggplot(data.frame(prevalence = wlightalc_range, PAF = walcprev$SCCpaf), aes(x = prevalence, y = PAF)) +
  geom_line(color = "#568259", size = 1) +
  labs(title = "Women",
       x = "Light Drinking Prevalence",
       y = "PAF") +
...
The assumptions behind this scenario are that all drinkers in England 2019 continue drinking but limit their consumption to light drinkers and there are no new drinkers entering the "exposed" population.

Here are the actual alcohol PAFs in 2019 vs. the PAFS with alcohol limiting assumptions (assumptions outlined above) for each cancer site.
...
```{r}
mpaf_bmi<- ggplot(data.frame(prevalence = moverweightrange2019, PAF = mbmiprev2019$ACpaf), aes(x = prevalence, y = PAF)) +
  geom_line(color = "#568259", size = 1) +
  labs(title = "Men",
  ...
  xlim(0.3,.7) +
  theme_minimal()
wpaf_bmi<- ggplot(data.frame(prevalence = woverweightrange2019, PAF = wbmiprev2019$ACpaf), aes(x = prevalence, y = PAF)) +
  geom_line(color = "#568259", size = 1) +
  labs(title = "Women",
       x = "Overweight Prevalence",
       y = "PAF") +
...
Here are the actual BMI PAFs in 2019 vs. the PAFS with BMI limiting assumptions (assumptions outlined above) for each cancer site.
...
One of the biggest discrepancies between current PAF literature and this analysis is the RR for fibre. The RR used in this analysis is 0.91(0.87, 1.00) per 10g increment of fibre per day and is sourced from a meta-analysis from the [WCRF](https://www.wcrf.org/wp-content/uploads/2020/12/Wholegrains-veg-and-fruit.pdf).The RR can be transformed to be an ERR per 1g/day by:\
$$
per~1~ g~ deficit  = ln(\frac1{0.93}) = \frac {0.073}{10g} / 10 = \frac {0.007}{1~g}
...
The PAF in Brown et al. for processed and red meat was calculated based on the population prevalence of red and processed meat together but the RR used is for processed meat - 74g/day of processed meat on average with an RR 1.13. This is an overestimate of the meat diet PAF calculations because not all of this 74g is processed meat.

In this analysis, processed meat and red meat are calculated separately with different RR and different prevalence from the NDNS based on IARC definitions of red and processed meat.
