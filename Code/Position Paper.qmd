...
# Read in data
fibre_data <- read_excel("Diet_Data.xlsx", sheet = "UK_Fibre")

# Clean data
fibre_data <- fibre_data |>
  filter(!is.na(`Fibre (g/day)4 5`)) |>
  select(-`Statistical comparison`, -...8, -...9)

# Get groups from data
fibre_data_groups <- fibre_data$`Fibre (g/day)4 5`[is.na(fibre_data$`NDNS RP survey years`)]

# Filter to variable of interest
fibre_data <- fibre_data |>
  filter(grepl(`Fibre (g/day)4 5`, pattern = "% meeting recommendation7")) 

# Create column names
colnames(fibre_data) <- c("variable", "2009", "2011", "2013", "2015", "2017")

# Add in groups
fibre_data$group <- fibre_data_groups

# Pivot longer
fibre_data <- fibre_data |>
  pivot_longer(cols = c("2009", "2011", "2013", "2015", "2017")) |>
  mutate(variable = "% Meeting Fibre Intake Recommendations") |>
  mutate(sex = str_extract(group, "^[^ ]+"),
         age_group = str_extract(group, "(?<= ).*")) |>
  filter(age_group == "19-64 years") |>
  mutate(sex = if_else(sex == "Adults", "All", sex),
         year = as.numeric(name),
         country = "UK",
         value = as.numeric(value))

# Create plot data
plot_data <- fibre_data |>
  filter(sex != "All")

# Plot figure
ggplot(plot_data, aes(x = year, y = value, colour = sex)) +
  geom_line() +
  geom_point() +
  xlab("Year") +
  ylab("% Meeting Fibre Intake Recommendations") +
  ggtitle("% Meeting Fibre Intake Recommendations in Adults in the UK") +
  labs(colour =  "Sex", caption = "*Adult defined as aged 19-64") +
  scale_x_continuous(breaks = 2002:2022, limits = c(2002, 2022)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        panel.grid.major.x = element_line(color = "grey92", size = 0.5),  # Adjust grid lines
        panel.grid.minor.x = element_blank())
